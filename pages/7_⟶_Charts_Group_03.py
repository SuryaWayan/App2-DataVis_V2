...
# Show charts?
Chart_3 = st.session_state.get('Chart_3', False)
def handle_Chart_3():
    if st.session_state.handle_Chart_3:
        st.session_state.Chart_3 = st.session_state.handle_Chart_3
Chart_3 = st.checkbox("Click to Show charts", Chart_3, on_change=handle_Chart_3, key='handle_Chart_3')
st.session_state.Chart_3 = Chart_3
...
if Chart_3:
    ...
    start_x_all_3 = st.session_state.get('start_x_all_3', None)
    end_x_all_3 = st.session_state.get('end_x_all_3', None)
    tunnel_3 = st.session_state.get('tunnel_3', False)
    threshold_3 = st.session_state.get('threshold_3', 0.00000)
    C_3_option = st.session_state.get('C_3_option', None)
    C_3 = st.session_state.get('C_3', 0.00000)
    C_3_column = st.session_state.get('C_3_column', "Row Number")
    B_3 = st.session_state.get('B_3', 0.00000)
    D_3 = st.session_state.get('D_3', 0.00000)
    A_3 = st.session_state.get('A_3', [])
    ...
    if data is not None:
        ...
        if start_x_all_3 is None or end_x_all_3 is None:
            start_x_all_3 = x_min
            end_x_all_3 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_3, col_x_range_end_3, col_remain_3 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_3:
            def handle_start_x_all_3():
                if st.session_state.handle_start_x_all_3:
                    st.session_state.start_x_all_3 = st.session_state.handle_start_x_all_3
            start_x_all_3 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_3, on_change=handle_start_x_all_3, key='handle_start_x_all_3')
        with col_x_range_end_3:
            def handle_end_x_all_3():
                if st.session_state.handle_end_x_all_3:
                    st.session_state.end_x_all_3 = st.session_state.handle_end_x_all_3
            end_x_all_3 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_3, on_change=handle_end_x_all_3, key='handle_end_x_all_3')
        st.session_state.start_x_all_3 = start_x_all_3
        st.session_state.end_x_all_3 = end_x_all_3
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_3, B_3, C_3_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_3_option == "User Input":
                    C_3_value = C_3
                elif C_3_option == "From Existing Variables":
                    if "C_3_column" in data.columns:
                        C_3_value = data["C_3_column"]
                    else:
                        st.error("Please select a column for C_3_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_3_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_3 + (B_3 * C_3_value / 100)
                upper_limit[column_data <= threshold] += D_3 + (B_3 * C_3_value / 100)
                upper_limit[column_data > threshold] += B_3 * C_3_value / 100
                lower_limit[column_data > threshold] -= B_3 * C_3_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_3():
            if st.session_state.handle_tunnel_3:
                st.session_state.tunnel_3 = st.session_state.handle_tunnel_3
        tunnel_3 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_3, on_change=handle_tunnel_3, key='handle_tunnel_3')
        st.session_state.tunnel_3 = tunnel_3
            
        if tunnel_3:
            col_notef_3, col_noteg_3 = st.columns([0.05,3.5])
            with col_noteg_3:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_3, col_notea_3, col_noteb_3, col_notec_3, col_noted_3, col_notee_3 = st.columns([0.08,1,1,1,1,1])

            with col_notea_3:
                def handle_threshold_3():
                        if st.session_state.handle_threshold_3:
                            st.session_state.threshold_3 = st.session_state.handle_threshold_3
                threshold_3 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_3, on_change=handle_threshold_3, key='handle_threshold_3')
                st.session_state.threshold_3 = threshold_3

            with col_noteb_3:
                def handle_C_3_option():
                        if st.session_state.handle_C_3_option:
                            st.session_state.C_3_option = st.session_state.handle_C_3_option
                C_3_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_3_option == "User Input" else 1, on_change=handle_C_3_option, key='handle_C_3_option')
                st.session_state.C_3_option = C_3_option
                if C_3_option == "User Input":
                    C_3 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_3)
                    st.session_state.C_3 = C_3

                elif C_3_option == "From Existing Variables":
                    #C_3_column = st.session_state.get('C_3_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_3_column)
                    def handle_C_3_column():
                        if st.session_state.handle_C_3_column:
                            st.session_state.C_3_column = st.session_state.handle_C_3_column
                    C_3_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_3_column, key='handle_C_3_column')
                    st.session_state['C_3_column'] = C_3_column
                    data["C_3_column"] = data[C_3_column]

            with col_notec_3:
                def handle_B_3():
                        if st.session_state.handle_B_3:
                            st.session_state.B_3 = st.session_state.handle_B_3
                B_3 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_3, on_change=handle_B_3, key='handle_B_3')
                st.session_state.B_3 = B_3

            with col_noted_3:
                def handle_D_3():
                        if st.session_state.handle_D_3:
                            st.session_state.D_3 = st.session_state.handle_D_3
                D_3 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_3, on_change=handle_D_3, key='handle_D_3')
                st.session_state.D_3 = D_3
                
            with col_notee_3:
                def handle_A_3():
                    if st.session_state.handle_A_3:
                        st.session_state.A_3 = st.session_state.handle_A_3
                A_3 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_3, on_change=handle_A_3, key='handle_A_3')    
                st.session_state.A_3 = A_3
            ...
            for column in A_3:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_3, D_3, B_3, C_3_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_3 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_3):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_3]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_3) & (data['Row Number'] <= end_x_all_3)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_3, y0=slope*start_x_all_3+intercept,
                                                    x1=end_x_all_3, y1=slope*end_x_all_3+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_3, end_x_all_3, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_3.append(data_row)

            summary_data_3.extend(chart_summary_data_3)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_3)
        st.session_state.summary_data_3 = summary_data_3
