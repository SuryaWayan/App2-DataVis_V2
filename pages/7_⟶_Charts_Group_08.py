...
# Show charts?
Chart_8 = st.session_state.get('Chart_8', False)
def handle_Chart_8():
    if st.session_state.handle_Chart_8:
        st.session_state.Chart_8 = st.session_state.handle_Chart_8
Chart_8 = st.checkbox("Click to Show charts", Chart_8, on_change=handle_Chart_8, key='handle_Chart_8')
st.session_state.Chart_8 = Chart_8
...
if Chart_8:
    ...
    start_x_all_8 = st.session_state.get('start_x_all_8', None)
    end_x_all_8 = st.session_state.get('end_x_all_8', None)
    tunnel_8 = st.session_state.get('tunnel_8', False)
    threshold_8 = st.session_state.get('threshold_8', 0.00000)
    C_8_option = st.session_state.get('C_8_option', None)
    C_8 = st.session_state.get('C_8', 0.00000)
    C_8_column = st.session_state.get('C_8_column', "Row Number")
    B_8 = st.session_state.get('B_8', 0.00000)
    D_8 = st.session_state.get('D_8', 0.00000)
    A_8 = st.session_state.get('A_8', [])
    ...
    if data is not None:
        ...
        if start_x_all_8 is None or end_x_all_8 is None:
            start_x_all_8 = x_min
            end_x_all_8 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_8, col_x_range_end_8, col_remain_8 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_8:
            def handle_start_x_all_8():
                if st.session_state.handle_start_x_all_8:
                    st.session_state.start_x_all_8 = st.session_state.handle_start_x_all_8
            start_x_all_8 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_8, on_change=handle_start_x_all_8, key='handle_start_x_all_8')
        with col_x_range_end_8:
            def handle_end_x_all_8():
                if st.session_state.handle_end_x_all_8:
                    st.session_state.end_x_all_8 = st.session_state.handle_end_x_all_8
            end_x_all_8 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_8, on_change=handle_end_x_all_8, key='handle_end_x_all_8')
        st.session_state.start_x_all_8 = start_x_all_8
        st.session_state.end_x_all_8 = end_x_all_8
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_8, B_8, C_8_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_8_option == "User Input":
                    C_8_value = C_8
                elif C_8_option == "From Existing Variables":
                    if "C_8_column" in data.columns:
                        C_8_value = data["C_8_column"]
                    else:
                        st.error("Please select a column for C_8_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_8_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_8 + (B_8 * C_8_value / 100)
                upper_limit[column_data <= threshold] += D_8 + (B_8 * C_8_value / 100)
                upper_limit[column_data > threshold] += B_8 * C_8_value / 100
                lower_limit[column_data > threshold] -= B_8 * C_8_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_8():
            if st.session_state.handle_tunnel_8:
                st.session_state.tunnel_8 = st.session_state.handle_tunnel_8
        tunnel_8 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_8, on_change=handle_tunnel_8, key='handle_tunnel_8')
        st.session_state.tunnel_8 = tunnel_8
            
        if tunnel_8:
            col_notef_8, col_noteg_8 = st.columns([0.05,3.5])
            with col_noteg_8:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_8, col_notea_8, col_noteb_8, col_notec_8, col_noted_8, col_notee_8 = st.columns([0.08,1,1,1,1,1])

            with col_notea_8:
                def handle_threshold_8():
                        if st.session_state.handle_threshold_8:
                            st.session_state.threshold_8 = st.session_state.handle_threshold_8
                threshold_8 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_8, on_change=handle_threshold_8, key='handle_threshold_8')
                st.session_state.threshold_8 = threshold_8

            with col_noteb_8:
                def handle_C_8_option():
                        if st.session_state.handle_C_8_option:
                            st.session_state.C_8_option = st.session_state.handle_C_8_option
                C_8_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_8_option == "User Input" else 1, on_change=handle_C_8_option, key='handle_C_8_option')
                st.session_state.C_8_option = C_8_option
                if C_8_option == "User Input":
                    C_8 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_8)
                    st.session_state.C_8 = C_8

                elif C_8_option == "From Existing Variables":
                    #C_8_column = st.session_state.get('C_8_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_8_column)
                    def handle_C_8_column():
                        if st.session_state.handle_C_8_column:
                            st.session_state.C_8_column = st.session_state.handle_C_8_column
                    C_8_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_8_column, key='handle_C_8_column')
                    st.session_state['C_8_column'] = C_8_column
                    data["C_8_column"] = data[C_8_column]

            with col_notec_8:
                def handle_B_8():
                        if st.session_state.handle_B_8:
                            st.session_state.B_8 = st.session_state.handle_B_8
                B_8 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_8, on_change=handle_B_8, key='handle_B_8')
                st.session_state.B_8 = B_8

            with col_noted_8:
                def handle_D_8():
                        if st.session_state.handle_D_8:
                            st.session_state.D_8 = st.session_state.handle_D_8
                D_8 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_8, on_change=handle_D_8, key='handle_D_8')
                st.session_state.D_8 = D_8
                
            with col_notee_8:
                def handle_A_8():
                    if st.session_state.handle_A_8:
                        st.session_state.A_8 = st.session_state.handle_A_8
                A_8 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_8, on_change=handle_A_8, key='handle_A_8')    
                st.session_state.A_8 = A_8
            ...
            for column in A_8:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_8, D_8, B_8, C_8_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_8 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_8):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_8]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_8) & (data['Row Number'] <= end_x_all_8)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_8, y0=slope*start_x_all_8+intercept,
                                                    x1=end_x_all_8, y1=slope*end_x_all_8+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_8, end_x_all_8, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_8.append(data_row)

            summary_data_8.extend(chart_summary_data_8)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_8)
        st.session_state.summary_data_8 = summary_data_8
