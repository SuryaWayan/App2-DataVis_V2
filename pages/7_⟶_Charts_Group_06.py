...
# Show charts?
Chart_6 = st.session_state.get('Chart_6', False)
def handle_Chart_6():
    if st.session_state.handle_Chart_6:
        st.session_state.Chart_6 = st.session_state.handle_Chart_6
Chart_6 = st.checkbox("Click to Show charts", Chart_6, on_change=handle_Chart_6, key='handle_Chart_6')
st.session_state.Chart_6 = Chart_6
...
if Chart_6:
    ...
    start_x_all_6 = st.session_state.get('start_x_all_6', None)
    end_x_all_6 = st.session_state.get('end_x_all_6', None)
    tunnel_6 = st.session_state.get('tunnel_6', False)
    threshold_6 = st.session_state.get('threshold_6', 0.00000)
    C_6_option = st.session_state.get('C_6_option', None)
    C_6 = st.session_state.get('C_6', 0.00000)
    C_6_column = st.session_state.get('C_6_column', "Row Number")
    B_6 = st.session_state.get('B_6', 0.00000)
    D_6 = st.session_state.get('D_6', 0.00000)
    A_6 = st.session_state.get('A_6', [])
    ...
    if data is not None:
        ...
        if start_x_all_6 is None or end_x_all_6 is None:
            start_x_all_6 = x_min
            end_x_all_6 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_6, col_x_range_end_6, col_remain_6 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_6:
            def handle_start_x_all_6():
                if st.session_state.handle_start_x_all_6:
                    st.session_state.start_x_all_6 = st.session_state.handle_start_x_all_6
            start_x_all_6 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_6, on_change=handle_start_x_all_6, key='handle_start_x_all_6')
        with col_x_range_end_6:
            def handle_end_x_all_6():
                if st.session_state.handle_end_x_all_6:
                    st.session_state.end_x_all_6 = st.session_state.handle_end_x_all_6
            end_x_all_6 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_6, on_change=handle_end_x_all_6, key='handle_end_x_all_6')
        st.session_state.start_x_all_6 = start_x_all_6
        st.session_state.end_x_all_6 = end_x_all_6
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_6, B_6, C_6_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_6_option == "User Input":
                    C_6_value = C_6
                elif C_6_option == "From Existing Variables":
                    if "C_6_column" in data.columns:
                        C_6_value = data["C_6_column"]
                    else:
                        st.error("Please select a column for C_6_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_6_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_6 + (B_6 * C_6_value / 100)
                upper_limit[column_data <= threshold] += D_6 + (B_6 * C_6_value / 100)
                upper_limit[column_data > threshold] += B_6 * C_6_value / 100
                lower_limit[column_data > threshold] -= B_6 * C_6_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_6():
            if st.session_state.handle_tunnel_6:
                st.session_state.tunnel_6 = st.session_state.handle_tunnel_6
        tunnel_6 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_6, on_change=handle_tunnel_6, key='handle_tunnel_6')
        st.session_state.tunnel_6 = tunnel_6
            
        if tunnel_6:
            col_notef_6, col_noteg_6 = st.columns([0.05,3.5])
            with col_noteg_6:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_6, col_notea_6, col_noteb_6, col_notec_6, col_noted_6, col_notee_6 = st.columns([0.08,1,1,1,1,1])

            with col_notea_6:
                def handle_threshold_6():
                        if st.session_state.handle_threshold_6:
                            st.session_state.threshold_6 = st.session_state.handle_threshold_6
                threshold_6 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_6, on_change=handle_threshold_6, key='handle_threshold_6')
                st.session_state.threshold_6 = threshold_6

            with col_noteb_6:
                def handle_C_6_option():
                        if st.session_state.handle_C_6_option:
                            st.session_state.C_6_option = st.session_state.handle_C_6_option
                C_6_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_6_option == "User Input" else 1, on_change=handle_C_6_option, key='handle_C_6_option')
                st.session_state.C_6_option = C_6_option
                if C_6_option == "User Input":
                    C_6 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_6)
                    st.session_state.C_6 = C_6

                elif C_6_option == "From Existing Variables":
                    #C_6_column = st.session_state.get('C_6_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_6_column)
                    def handle_C_6_column():
                        if st.session_state.handle_C_6_column:
                            st.session_state.C_6_column = st.session_state.handle_C_6_column
                    C_6_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_6_column, key='handle_C_6_column')
                    st.session_state['C_6_column'] = C_6_column
                    data["C_6_column"] = data[C_6_column]

            with col_notec_6:
                def handle_B_6():
                        if st.session_state.handle_B_6:
                            st.session_state.B_6 = st.session_state.handle_B_6
                B_6 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_6, on_change=handle_B_6, key='handle_B_6')
                st.session_state.B_6 = B_6

            with col_noted_6:
                def handle_D_6():
                        if st.session_state.handle_D_6:
                            st.session_state.D_6 = st.session_state.handle_D_6
                D_6 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_6, on_change=handle_D_6, key='handle_D_6')
                st.session_state.D_6 = D_6
                
            with col_notee_6:
                def handle_A_6():
                    if st.session_state.handle_A_6:
                        st.session_state.A_6 = st.session_state.handle_A_6
                A_6 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_6, on_change=handle_A_6, key='handle_A_6')    
                st.session_state.A_6 = A_6
            ...
            for column in A_6:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_6, D_6, B_6, C_6_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_6 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_6):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_6]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_6) & (data['Row Number'] <= end_x_all_6)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_6, y0=slope*start_x_all_6+intercept,
                                                    x1=end_x_all_6, y1=slope*end_x_all_6+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_6, end_x_all_6, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_6.append(data_row)

            summary_data_6.extend(chart_summary_data_6)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_6)
        st.session_state.summary_data_6 = summary_data_6
