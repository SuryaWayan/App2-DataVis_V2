...
# Show charts?
Chart_13 = st.session_state.get('Chart_13', False)
def handle_Chart_13():
    if st.session_state.handle_Chart_13:
        st.session_state.Chart_13 = st.session_state.handle_Chart_13
Chart_13 = st.checkbox("Click to Show charts", Chart_13, on_change=handle_Chart_13, key='handle_Chart_13')
st.session_state.Chart_13 = Chart_13
...
if Chart_13:
    ...
    start_x_all_13 = st.session_state.get('start_x_all_13', None)
    end_x_all_13 = st.session_state.get('end_x_all_13', None)
    tunnel_13 = st.session_state.get('tunnel_13', False)
    threshold_13 = st.session_state.get('threshold_13', 0.00000)
    C_13_option = st.session_state.get('C_13_option', None)
    C_13 = st.session_state.get('C_13', 0.00000)
    C_13_column = st.session_state.get('C_13_column', "Row Number")
    B_13 = st.session_state.get('B_13', 0.00000)
    D_13 = st.session_state.get('D_13', 0.00000)
    A_13 = st.session_state.get('A_13', [])
    ...
    if data is not None:
        ...
        if start_x_all_13 is None or end_x_all_13 is None:
            start_x_all_13 = x_min
            end_x_all_13 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_13, col_x_range_end_13, col_remain_13 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_13:
            def handle_start_x_all_13():
                if st.session_state.handle_start_x_all_13:
                    st.session_state.start_x_all_13 = st.session_state.handle_start_x_all_13
            start_x_all_13 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_13, on_change=handle_start_x_all_13, key='handle_start_x_all_13')
        with col_x_range_end_13:
            def handle_end_x_all_13():
                if st.session_state.handle_end_x_all_13:
                    st.session_state.end_x_all_13 = st.session_state.handle_end_x_all_13
            end_x_all_13 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_13, on_change=handle_end_x_all_13, key='handle_end_x_all_13')
        st.session_state.start_x_all_13 = start_x_all_13
        st.session_state.end_x_all_13 = end_x_all_13
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_13, B_13, C_13_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_13_option == "User Input":
                    C_13_value = C_13
                elif C_13_option == "From Existing Variables":
                    if "C_13_column" in data.columns:
                        C_13_value = data["C_13_column"]
                    else:
                        st.error("Please select a column for C_13_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_13_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_13 + (B_13 * C_13_value / 100)
                upper_limit[column_data <= threshold] += D_13 + (B_13 * C_13_value / 100)
                upper_limit[column_data > threshold] += B_13 * C_13_value / 100
                lower_limit[column_data > threshold] -= B_13 * C_13_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_13():
            if st.session_state.handle_tunnel_13:
                st.session_state.tunnel_13 = st.session_state.handle_tunnel_13
        tunnel_13 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_13, on_change=handle_tunnel_13, key='handle_tunnel_13')
        st.session_state.tunnel_13 = tunnel_13
            
        if tunnel_13:
            col_notef_13, col_noteg_13 = st.columns([0.05,3.5])
            with col_noteg_13:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_13, col_notea_13, col_noteb_13, col_notec_13, col_noted_13, col_notee_13 = st.columns([0.08,1,1,1,1,1])

            with col_notea_13:
                def handle_threshold_13():
                        if st.session_state.handle_threshold_13:
                            st.session_state.threshold_13 = st.session_state.handle_threshold_13
                threshold_13 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_13, on_change=handle_threshold_13, key='handle_threshold_13')
                st.session_state.threshold_13 = threshold_13

            with col_noteb_13:
                def handle_C_13_option():
                        if st.session_state.handle_C_13_option:
                            st.session_state.C_13_option = st.session_state.handle_C_13_option
                C_13_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_13_option == "User Input" else 1, on_change=handle_C_13_option, key='handle_C_13_option')
                st.session_state.C_13_option = C_13_option
                if C_13_option == "User Input":
                    C_13 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_13)
                    st.session_state.C_13 = C_13

                elif C_13_option == "From Existing Variables":
                    #C_13_column = st.session_state.get('C_13_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_13_column)
                    def handle_C_13_column():
                        if st.session_state.handle_C_13_column:
                            st.session_state.C_13_column = st.session_state.handle_C_13_column
                    C_13_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_13_column, key='handle_C_13_column')
                    st.session_state['C_13_column'] = C_13_column
                    data["C_13_column"] = data[C_13_column]

            with col_notec_13:
                def handle_B_13():
                        if st.session_state.handle_B_13:
                            st.session_state.B_13 = st.session_state.handle_B_13
                B_13 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_13, on_change=handle_B_13, key='handle_B_13')
                st.session_state.B_13 = B_13

            with col_noted_13:
                def handle_D_13():
                        if st.session_state.handle_D_13:
                            st.session_state.D_13 = st.session_state.handle_D_13
                D_13 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_13, on_change=handle_D_13, key='handle_D_13')
                st.session_state.D_13 = D_13
                
            with col_notee_13:
                def handle_A_13():
                    if st.session_state.handle_A_13:
                        st.session_state.A_13 = st.session_state.handle_A_13
                A_13 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_13, on_change=handle_A_13, key='handle_A_13')    
                st.session_state.A_13 = A_13
            ...
            for column in A_13:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_13, D_13, B_13, C_13_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_13 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_13):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_13]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_13) & (data['Row Number'] <= end_x_all_13)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_13, y0=slope*start_x_all_13+intercept,
                                                    x1=end_x_all_13, y1=slope*end_x_all_13+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_13, end_x_all_13, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_13.append(data_row)

            summary_data_13.extend(chart_summary_data_13)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_13)
        st.session_state.summary_data_13 = summary_data_13
