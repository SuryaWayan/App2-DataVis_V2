...
# Show charts?
Chart_4 = st.session_state.get('Chart_4', False)
def handle_Chart_4():
    if st.session_state.handle_Chart_4:
        st.session_state.Chart_4 = st.session_state.handle_Chart_4
Chart_4 = st.checkbox("Click to Show charts", Chart_4, on_change=handle_Chart_4, key='handle_Chart_4')
st.session_state.Chart_4 = Chart_4
...
if Chart_4:
    ...
    start_x_all_4 = st.session_state.get('start_x_all_4', None)
    end_x_all_4 = st.session_state.get('end_x_all_4', None)
    tunnel_4 = st.session_state.get('tunnel_4', False)
    threshold_4 = st.session_state.get('threshold_4', 0.00000)
    C_4_option = st.session_state.get('C_4_option', None)
    C_4 = st.session_state.get('C_4', 0.00000)
    C_4_column = st.session_state.get('C_4_column', "Row Number")
    B_4 = st.session_state.get('B_4', 0.00000)
    D_4 = st.session_state.get('D_4', 0.00000)
    A_4 = st.session_state.get('A_4', [])
    ...
    if data is not None:
        ...
        if start_x_all_4 is None or end_x_all_4 is None:
            start_x_all_4 = x_min
            end_x_all_4 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_4, col_x_range_end_4, col_remain_4 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_4:
            def handle_start_x_all_4():
                if st.session_state.handle_start_x_all_4:
                    st.session_state.start_x_all_4 = st.session_state.handle_start_x_all_4
            start_x_all_4 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_4, on_change=handle_start_x_all_4, key='handle_start_x_all_4')
        with col_x_range_end_4:
            def handle_end_x_all_4():
                if st.session_state.handle_end_x_all_4:
                    st.session_state.end_x_all_4 = st.session_state.handle_end_x_all_4
            end_x_all_4 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_4, on_change=handle_end_x_all_4, key='handle_end_x_all_4')
        st.session_state.start_x_all_4 = start_x_all_4
        st.session_state.end_x_all_4 = end_x_all_4
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_4, B_4, C_4_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_4_option == "User Input":
                    C_4_value = C_4
                elif C_4_option == "From Existing Variables":
                    if "C_4_column" in data.columns:
                        C_4_value = data["C_4_column"]
                    else:
                        st.error("Please select a column for C_4_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_4_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_4 + (B_4 * C_4_value / 100)
                upper_limit[column_data <= threshold] += D_4 + (B_4 * C_4_value / 100)
                upper_limit[column_data > threshold] += B_4 * C_4_value / 100
                lower_limit[column_data > threshold] -= B_4 * C_4_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_4():
            if st.session_state.handle_tunnel_4:
                st.session_state.tunnel_4 = st.session_state.handle_tunnel_4
        tunnel_4 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_4, on_change=handle_tunnel_4, key='handle_tunnel_4')
        st.session_state.tunnel_4 = tunnel_4
            
        if tunnel_4:
            col_notef_4, col_noteg_4 = st.columns([0.05,3.5])
            with col_noteg_4:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_4, col_notea_4, col_noteb_4, col_notec_4, col_noted_4, col_notee_4 = st.columns([0.08,1,1,1,1,1])

            with col_notea_4:
                def handle_threshold_4():
                        if st.session_state.handle_threshold_4:
                            st.session_state.threshold_4 = st.session_state.handle_threshold_4
                threshold_4 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_4, on_change=handle_threshold_4, key='handle_threshold_4')
                st.session_state.threshold_4 = threshold_4

            with col_noteb_4:
                def handle_C_4_option():
                        if st.session_state.handle_C_4_option:
                            st.session_state.C_4_option = st.session_state.handle_C_4_option
                C_4_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_4_option == "User Input" else 1, on_change=handle_C_4_option, key='handle_C_4_option')
                st.session_state.C_4_option = C_4_option
                if C_4_option == "User Input":
                    C_4 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_4)
                    st.session_state.C_4 = C_4

                elif C_4_option == "From Existing Variables":
                    #C_4_column = st.session_state.get('C_4_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_4_column)
                    def handle_C_4_column():
                        if st.session_state.handle_C_4_column:
                            st.session_state.C_4_column = st.session_state.handle_C_4_column
                    C_4_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_4_column, key='handle_C_4_column')
                    st.session_state['C_4_column'] = C_4_column
                    data["C_4_column"] = data[C_4_column]

            with col_notec_4:
                def handle_B_4():
                        if st.session_state.handle_B_4:
                            st.session_state.B_4 = st.session_state.handle_B_4
                B_4 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_4, on_change=handle_B_4, key='handle_B_4')
                st.session_state.B_4 = B_4

            with col_noted_4:
                def handle_D_4():
                        if st.session_state.handle_D_4:
                            st.session_state.D_4 = st.session_state.handle_D_4
                D_4 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_4, on_change=handle_D_4, key='handle_D_4')
                st.session_state.D_4 = D_4
                
            with col_notee_4:
                def handle_A_4():
                    if st.session_state.handle_A_4:
                        st.session_state.A_4 = st.session_state.handle_A_4
                A_4 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_4, on_change=handle_A_4, key='handle_A_4')    
                st.session_state.A_4 = A_4
            ...
            for column in A_4:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_4, D_4, B_4, C_4_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_4 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_4):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_4]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_4) & (data['Row Number'] <= end_x_all_4)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_4, y0=slope*start_x_all_4+intercept,
                                                    x1=end_x_all_4, y1=slope*end_x_all_4+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_4, end_x_all_4, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_4.append(data_row)

            summary_data_4.extend(chart_summary_data_4)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_4)
        st.session_state.summary_data_4 = summary_data_4
