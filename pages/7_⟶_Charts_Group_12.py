...
# Show charts?
Chart_12 = st.session_state.get('Chart_12', False)
def handle_Chart_12():
    if st.session_state.handle_Chart_12:
        st.session_state.Chart_12 = st.session_state.handle_Chart_12
Chart_12 = st.checkbox("Click to Show charts", Chart_12, on_change=handle_Chart_12, key='handle_Chart_12')
st.session_state.Chart_12 = Chart_12
...
if Chart_12:
    ...
    start_x_all_12 = st.session_state.get('start_x_all_12', None)
    end_x_all_12 = st.session_state.get('end_x_all_12', None)
    tunnel_12 = st.session_state.get('tunnel_12', False)
    threshold_12 = st.session_state.get('threshold_12', 0.00000)
    C_12_option = st.session_state.get('C_12_option', None)
    C_12 = st.session_state.get('C_12', 0.00000)
    C_12_column = st.session_state.get('C_12_column', "Row Number")
    B_12 = st.session_state.get('B_12', 0.00000)
    D_12 = st.session_state.get('D_12', 0.00000)
    A_12 = st.session_state.get('A_12', [])
    ...
    if data is not None:
        ...
        if start_x_all_12 is None or end_x_all_12 is None:
            start_x_all_12 = x_min
            end_x_all_12 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_12, col_x_range_end_12, col_remain_12 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_12:
            def handle_start_x_all_12():
                if st.session_state.handle_start_x_all_12:
                    st.session_state.start_x_all_12 = st.session_state.handle_start_x_all_12
            start_x_all_12 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_12, on_change=handle_start_x_all_12, key='handle_start_x_all_12')
        with col_x_range_end_12:
            def handle_end_x_all_12():
                if st.session_state.handle_end_x_all_12:
                    st.session_state.end_x_all_12 = st.session_state.handle_end_x_all_12
            end_x_all_12 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_12, on_change=handle_end_x_all_12, key='handle_end_x_all_12')
        st.session_state.start_x_all_12 = start_x_all_12
        st.session_state.end_x_all_12 = end_x_all_12
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_12, B_12, C_12_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_12_option == "User Input":
                    C_12_value = C_12
                elif C_12_option == "From Existing Variables":
                    if "C_12_column" in data.columns:
                        C_12_value = data["C_12_column"]
                    else:
                        st.error("Please select a column for C_12_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_12_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_12 + (B_12 * C_12_value / 100)
                upper_limit[column_data <= threshold] += D_12 + (B_12 * C_12_value / 100)
                upper_limit[column_data > threshold] += B_12 * C_12_value / 100
                lower_limit[column_data > threshold] -= B_12 * C_12_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_12():
            if st.session_state.handle_tunnel_12:
                st.session_state.tunnel_12 = st.session_state.handle_tunnel_12
        tunnel_12 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_12, on_change=handle_tunnel_12, key='handle_tunnel_12')
        st.session_state.tunnel_12 = tunnel_12
            
        if tunnel_12:
            col_notef_12, col_noteg_12 = st.columns([0.05,3.5])
            with col_noteg_12:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_12, col_notea_12, col_noteb_12, col_notec_12, col_noted_12, col_notee_12 = st.columns([0.08,1,1,1,1,1])

            with col_notea_12:
                def handle_threshold_12():
                        if st.session_state.handle_threshold_12:
                            st.session_state.threshold_12 = st.session_state.handle_threshold_12
                threshold_12 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_12, on_change=handle_threshold_12, key='handle_threshold_12')
                st.session_state.threshold_12 = threshold_12

            with col_noteb_12:
                def handle_C_12_option():
                        if st.session_state.handle_C_12_option:
                            st.session_state.C_12_option = st.session_state.handle_C_12_option
                C_12_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_12_option == "User Input" else 1, on_change=handle_C_12_option, key='handle_C_12_option')
                st.session_state.C_12_option = C_12_option
                if C_12_option == "User Input":
                    C_12 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_12)
                    st.session_state.C_12 = C_12

                elif C_12_option == "From Existing Variables":
                    #C_12_column = st.session_state.get('C_12_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_12_column)
                    def handle_C_12_column():
                        if st.session_state.handle_C_12_column:
                            st.session_state.C_12_column = st.session_state.handle_C_12_column
                    C_12_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_12_column, key='handle_C_12_column')
                    st.session_state['C_12_column'] = C_12_column
                    data["C_12_column"] = data[C_12_column]

            with col_notec_12:
                def handle_B_12():
                        if st.session_state.handle_B_12:
                            st.session_state.B_12 = st.session_state.handle_B_12
                B_12 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_12, on_change=handle_B_12, key='handle_B_12')
                st.session_state.B_12 = B_12

            with col_noted_12:
                def handle_D_12():
                        if st.session_state.handle_D_12:
                            st.session_state.D_12 = st.session_state.handle_D_12
                D_12 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_12, on_change=handle_D_12, key='handle_D_12')
                st.session_state.D_12 = D_12
                
            with col_notee_12:
                def handle_A_12():
                    if st.session_state.handle_A_12:
                        st.session_state.A_12 = st.session_state.handle_A_12
                A_12 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_12, on_change=handle_A_12, key='handle_A_12')    
                st.session_state.A_12 = A_12
            ...
            for column in A_12:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_12, D_12, B_12, C_12_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_12 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_12):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_12]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_12) & (data['Row Number'] <= end_x_all_12)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_12, y0=slope*start_x_all_12+intercept,
                                                    x1=end_x_all_12, y1=slope*end_x_all_12+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_12, end_x_all_12, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_12.append(data_row)

            summary_data_12.extend(chart_summary_data_12)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_12)
        st.session_state.summary_data_12 = summary_data_12
