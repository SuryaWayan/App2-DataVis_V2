...
# Show charts?
Chart_11 = st.session_state.get('Chart_11', False)
def handle_Chart_11():
    if st.session_state.handle_Chart_11:
        st.session_state.Chart_11 = st.session_state.handle_Chart_11
Chart_11 = st.checkbox("Click to Show charts", Chart_11, on_change=handle_Chart_11, key='handle_Chart_11')
st.session_state.Chart_11 = Chart_11
...
if Chart_11:
    ...
    start_x_all_11 = st.session_state.get('start_x_all_11', None)
    end_x_all_11 = st.session_state.get('end_x_all_11', None)
    tunnel_11 = st.session_state.get('tunnel_11', False)
    threshold_11 = st.session_state.get('threshold_11', 0.00000)
    C_11_option = st.session_state.get('C_11_option', None)
    C_11 = st.session_state.get('C_11', 0.00000)
    C_11_column = st.session_state.get('C_11_column', "Row Number")
    B_11 = st.session_state.get('B_11', 0.00000)
    D_11 = st.session_state.get('D_11', 0.00000)
    A_11 = st.session_state.get('A_11', [])
    ...
    if data is not None:
        ...
        if start_x_all_11 is None or end_x_all_11 is None:
            start_x_all_11 = x_min
            end_x_all_11 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_11, col_x_range_end_11, col_remain_11 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_11:
            def handle_start_x_all_11():
                if st.session_state.handle_start_x_all_11:
                    st.session_state.start_x_all_11 = st.session_state.handle_start_x_all_11
            start_x_all_11 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_11, on_change=handle_start_x_all_11, key='handle_start_x_all_11')
        with col_x_range_end_11:
            def handle_end_x_all_11():
                if st.session_state.handle_end_x_all_11:
                    st.session_state.end_x_all_11 = st.session_state.handle_end_x_all_11
            end_x_all_11 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_11, on_change=handle_end_x_all_11, key='handle_end_x_all_11')
        st.session_state.start_x_all_11 = start_x_all_11
        st.session_state.end_x_all_11 = end_x_all_11
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_11, B_11, C_11_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_11_option == "User Input":
                    C_11_value = C_11
                elif C_11_option == "From Existing Variables":
                    if "C_11_column" in data.columns:
                        C_11_value = data["C_11_column"]
                    else:
                        st.error("Please select a column for C_11_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_11_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_11 + (B_11 * C_11_value / 100)
                upper_limit[column_data <= threshold] += D_11 + (B_11 * C_11_value / 100)
                upper_limit[column_data > threshold] += B_11 * C_11_value / 100
                lower_limit[column_data > threshold] -= B_11 * C_11_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_11():
            if st.session_state.handle_tunnel_11:
                st.session_state.tunnel_11 = st.session_state.handle_tunnel_11
        tunnel_11 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_11, on_change=handle_tunnel_11, key='handle_tunnel_11')
        st.session_state.tunnel_11 = tunnel_11
            
        if tunnel_11:
            col_notef_11, col_noteg_11 = st.columns([0.05,3.5])
            with col_noteg_11:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_11, col_notea_11, col_noteb_11, col_notec_11, col_noted_11, col_notee_11 = st.columns([0.08,1,1,1,1,1])

            with col_notea_11:
                def handle_threshold_11():
                        if st.session_state.handle_threshold_11:
                            st.session_state.threshold_11 = st.session_state.handle_threshold_11
                threshold_11 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_11, on_change=handle_threshold_11, key='handle_threshold_11')
                st.session_state.threshold_11 = threshold_11

            with col_noteb_11:
                def handle_C_11_option():
                        if st.session_state.handle_C_11_option:
                            st.session_state.C_11_option = st.session_state.handle_C_11_option
                C_11_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_11_option == "User Input" else 1, on_change=handle_C_11_option, key='handle_C_11_option')
                st.session_state.C_11_option = C_11_option
                if C_11_option == "User Input":
                    C_11 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_11)
                    st.session_state.C_11 = C_11

                elif C_11_option == "From Existing Variables":
                    #C_11_column = st.session_state.get('C_11_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_11_column)
                    def handle_C_11_column():
                        if st.session_state.handle_C_11_column:
                            st.session_state.C_11_column = st.session_state.handle_C_11_column
                    C_11_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_11_column, key='handle_C_11_column')
                    st.session_state['C_11_column'] = C_11_column
                    data["C_11_column"] = data[C_11_column]

            with col_notec_11:
                def handle_B_11():
                        if st.session_state.handle_B_11:
                            st.session_state.B_11 = st.session_state.handle_B_11
                B_11 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_11, on_change=handle_B_11, key='handle_B_11')
                st.session_state.B_11 = B_11

            with col_noted_11:
                def handle_D_11():
                        if st.session_state.handle_D_11:
                            st.session_state.D_11 = st.session_state.handle_D_11
                D_11 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_11, on_change=handle_D_11, key='handle_D_11')
                st.session_state.D_11 = D_11
                
            with col_notee_11:
                def handle_A_11():
                    if st.session_state.handle_A_11:
                        st.session_state.A_11 = st.session_state.handle_A_11
                A_11 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_11, on_change=handle_A_11, key='handle_A_11')    
                st.session_state.A_11 = A_11
            ...
            for column in A_11:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_11, D_11, B_11, C_11_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_11 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_11):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_11]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_11) & (data['Row Number'] <= end_x_all_11)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_11, y0=slope*start_x_all_11+intercept,
                                                    x1=end_x_all_11, y1=slope*end_x_all_11+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_11, end_x_all_11, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_11.append(data_row)

            summary_data_11.extend(chart_summary_data_11)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_11)
        st.session_state.summary_data_11 = summary_data_11
