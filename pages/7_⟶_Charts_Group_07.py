...
# Show charts?
Chart_7 = st.session_state.get('Chart_7', False)
def handle_Chart_7():
    if st.session_state.handle_Chart_7:
        st.session_state.Chart_7 = st.session_state.handle_Chart_7
Chart_7 = st.checkbox("Click to Show charts", Chart_7, on_change=handle_Chart_7, key='handle_Chart_7')
st.session_state.Chart_7 = Chart_7
...
if Chart_7:
    ...
    start_x_all_7 = st.session_state.get('start_x_all_7', None)
    end_x_all_7 = st.session_state.get('end_x_all_7', None)
    tunnel_7 = st.session_state.get('tunnel_7', False)
    threshold_7 = st.session_state.get('threshold_7', 0.00000)
    C_7_option = st.session_state.get('C_7_option', None)
    C_7 = st.session_state.get('C_7', 0.00000)
    C_7_column = st.session_state.get('C_7_column', "Row Number")
    B_7 = st.session_state.get('B_7', 0.00000)
    D_7 = st.session_state.get('D_7', 0.00000)
    A_7 = st.session_state.get('A_7', [])
    ...
    if data is not None:
        ...
        if start_x_all_7 is None or end_x_all_7 is None:
            start_x_all_7 = x_min
            end_x_all_7 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_7, col_x_range_end_7, col_remain_7 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_7:
            def handle_start_x_all_7():
                if st.session_state.handle_start_x_all_7:
                    st.session_state.start_x_all_7 = st.session_state.handle_start_x_all_7
            start_x_all_7 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_7, on_change=handle_start_x_all_7, key='handle_start_x_all_7')
        with col_x_range_end_7:
            def handle_end_x_all_7():
                if st.session_state.handle_end_x_all_7:
                    st.session_state.end_x_all_7 = st.session_state.handle_end_x_all_7
            end_x_all_7 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_7, on_change=handle_end_x_all_7, key='handle_end_x_all_7')
        st.session_state.start_x_all_7 = start_x_all_7
        st.session_state.end_x_all_7 = end_x_all_7
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_7, B_7, C_7_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_7_option == "User Input":
                    C_7_value = C_7
                elif C_7_option == "From Existing Variables":
                    if "C_7_column" in data.columns:
                        C_7_value = data["C_7_column"]
                    else:
                        st.error("Please select a column for C_7_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_7_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_7 + (B_7 * C_7_value / 100)
                upper_limit[column_data <= threshold] += D_7 + (B_7 * C_7_value / 100)
                upper_limit[column_data > threshold] += B_7 * C_7_value / 100
                lower_limit[column_data > threshold] -= B_7 * C_7_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_7():
            if st.session_state.handle_tunnel_7:
                st.session_state.tunnel_7 = st.session_state.handle_tunnel_7
        tunnel_7 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_7, on_change=handle_tunnel_7, key='handle_tunnel_7')
        st.session_state.tunnel_7 = tunnel_7
            
        if tunnel_7:
            col_notef_7, col_noteg_7 = st.columns([0.05,3.5])
            with col_noteg_7:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_7, col_notea_7, col_noteb_7, col_notec_7, col_noted_7, col_notee_7 = st.columns([0.08,1,1,1,1,1])

            with col_notea_7:
                def handle_threshold_7():
                        if st.session_state.handle_threshold_7:
                            st.session_state.threshold_7 = st.session_state.handle_threshold_7
                threshold_7 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_7, on_change=handle_threshold_7, key='handle_threshold_7')
                st.session_state.threshold_7 = threshold_7

            with col_noteb_7:
                def handle_C_7_option():
                        if st.session_state.handle_C_7_option:
                            st.session_state.C_7_option = st.session_state.handle_C_7_option
                C_7_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_7_option == "User Input" else 1, on_change=handle_C_7_option, key='handle_C_7_option')
                st.session_state.C_7_option = C_7_option
                if C_7_option == "User Input":
                    C_7 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_7)
                    st.session_state.C_7 = C_7

                elif C_7_option == "From Existing Variables":
                    #C_7_column = st.session_state.get('C_7_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_7_column)
                    def handle_C_7_column():
                        if st.session_state.handle_C_7_column:
                            st.session_state.C_7_column = st.session_state.handle_C_7_column
                    C_7_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_7_column, key='handle_C_7_column')
                    st.session_state['C_7_column'] = C_7_column
                    data["C_7_column"] = data[C_7_column]

            with col_notec_7:
                def handle_B_7():
                        if st.session_state.handle_B_7:
                            st.session_state.B_7 = st.session_state.handle_B_7
                B_7 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_7, on_change=handle_B_7, key='handle_B_7')
                st.session_state.B_7 = B_7

            with col_noted_7:
                def handle_D_7():
                        if st.session_state.handle_D_7:
                            st.session_state.D_7 = st.session_state.handle_D_7
                D_7 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_7, on_change=handle_D_7, key='handle_D_7')
                st.session_state.D_7 = D_7
                
            with col_notee_7:
                def handle_A_7():
                    if st.session_state.handle_A_7:
                        st.session_state.A_7 = st.session_state.handle_A_7
                A_7 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_7, on_change=handle_A_7, key='handle_A_7')    
                st.session_state.A_7 = A_7
            ...
            for column in A_7:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_7, D_7, B_7, C_7_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_7 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_7):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_7]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_7) & (data['Row Number'] <= end_x_all_7)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_7, y0=slope*start_x_all_7+intercept,
                                                    x1=end_x_all_7, y1=slope*end_x_all_7+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_7, end_x_all_7, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_7.append(data_row)

            summary_data_7.extend(chart_summary_data_7)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_7)
        st.session_state.summary_data_7 = summary_data_7
