...
# Show charts?
Chart_2 = st.session_state.get('Chart_2', False)
def handle_Chart_2():
    if st.session_state.handle_Chart_2:
        st.session_state.Chart_2 = st.session_state.handle_Chart_2
Chart_2 = st.checkbox("Click to Show charts", Chart_2, on_change=handle_Chart_2, key='handle_Chart_2')
st.session_state.Chart_2 = Chart_2
...
if Chart_2:
    ...
    start_x_all_2 = st.session_state.get('start_x_all_2', None)
    end_x_all_2 = st.session_state.get('end_x_all_2', None)
    tunnel_2 = st.session_state.get('tunnel_2', False)
    threshold_2 = st.session_state.get('threshold_2', 0.00000)
    C_2_option = st.session_state.get('C_2_option', None)
    C_2 = st.session_state.get('C_2', 0.00000)
    C_2_column = st.session_state.get('C_2_column', "Row Number")
    B_2 = st.session_state.get('B_2', 0.00000)
    D_2 = st.session_state.get('D_2', 0.00000)
    A_2 = st.session_state.get('A_2', [])
    ...
    if data is not None:
        ...
        if start_x_all_2 is None or end_x_all_2 is None:
            start_x_all_2 = x_min
            end_x_all_2 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_2, col_x_range_end_2, col_remain_2 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_2:
            def handle_start_x_all_2():
                if st.session_state.handle_start_x_all_2:
                    st.session_state.start_x_all_2 = st.session_state.handle_start_x_all_2
            start_x_all_2 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_2, on_change=handle_start_x_all_2, key='handle_start_x_all_2')
        with col_x_range_end_2:
            def handle_end_x_all_2():
                if st.session_state.handle_end_x_all_2:
                    st.session_state.end_x_all_2 = st.session_state.handle_end_x_all_2
            end_x_all_2 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_2, on_change=handle_end_x_all_2, key='handle_end_x_all_2')
        st.session_state.start_x_all_2 = start_x_all_2
        st.session_state.end_x_all_2 = end_x_all_2
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_2, B_2, C_2_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_2_option == "User Input":
                    C_2_value = C_2
                elif C_2_option == "From Existing Variables":
                    if "C_2_column" in data.columns:
                        C_2_value = data["C_2_column"]
                    else:
                        st.error("Please select a column for C_2_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_2_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_2 + (B_2 * C_2_value / 100)
                upper_limit[column_data <= threshold] += D_2 + (B_2 * C_2_value / 100)
                upper_limit[column_data > threshold] += B_2 * C_2_value / 100
                lower_limit[column_data > threshold] -= B_2 * C_2_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_2():
            if st.session_state.handle_tunnel_2:
                st.session_state.tunnel_2 = st.session_state.handle_tunnel_2
        tunnel_2 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_2, on_change=handle_tunnel_2, key='handle_tunnel_2')
        st.session_state.tunnel_2 = tunnel_2
            
        if tunnel_2:
            col_notef_2, col_noteg_2 = st.columns([0.05,3.5])
            with col_noteg_2:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_2, col_notea_2, col_noteb_2, col_notec_2, col_noted_2, col_notee_2 = st.columns([0.08,1,1,1,1,1])

            with col_notea_2:
                def handle_threshold_2():
                        if st.session_state.handle_threshold_2:
                            st.session_state.threshold_2 = st.session_state.handle_threshold_2
                threshold_2 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_2, on_change=handle_threshold_2, key='handle_threshold_2')
                st.session_state.threshold_2 = threshold_2

            with col_noteb_2:
                def handle_C_2_option():
                        if st.session_state.handle_C_2_option:
                            st.session_state.C_2_option = st.session_state.handle_C_2_option
                C_2_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_2_option == "User Input" else 1, on_change=handle_C_2_option, key='handle_C_2_option')
                st.session_state.C_2_option = C_2_option
                if C_2_option == "User Input":
                    C_2 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_2)
                    st.session_state.C_2 = C_2

                elif C_2_option == "From Existing Variables":
                    #C_2_column = st.session_state.get('C_2_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_2_column)
                    def handle_C_2_column():
                        if st.session_state.handle_C_2_column:
                            st.session_state.C_2_column = st.session_state.handle_C_2_column
                    C_2_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_2_column, key='handle_C_2_column')
                    st.session_state['C_2_column'] = C_2_column
                    data["C_2_column"] = data[C_2_column]

            with col_notec_2:
                def handle_B_2():
                        if st.session_state.handle_B_2:
                            st.session_state.B_2 = st.session_state.handle_B_2
                B_2 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_2, on_change=handle_B_2, key='handle_B_2')
                st.session_state.B_2 = B_2

            with col_noted_2:
                def handle_D_2():
                        if st.session_state.handle_D_2:
                            st.session_state.D_2 = st.session_state.handle_D_2
                D_2 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_2, on_change=handle_D_2, key='handle_D_2')
                st.session_state.D_2 = D_2
                
            with col_notee_2:
                def handle_A_2():
                    if st.session_state.handle_A_2:
                        st.session_state.A_2 = st.session_state.handle_A_2
                A_2 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_2, on_change=handle_A_2, key='handle_A_2')    
                st.session_state.A_2 = A_2
            ...
            for column in A_2:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_2, D_2, B_2, C_2_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_2 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_2):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_2]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_2) & (data['Row Number'] <= end_x_all_2)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_2, y0=slope*start_x_all_2+intercept,
                                                    x1=end_x_all_2, y1=slope*end_x_all_2+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_2, end_x_all_2, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_2.append(data_row)

            summary_data_2.extend(chart_summary_data_2)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_2)
        st.session_state.summary_data_2 = summary_data_2
