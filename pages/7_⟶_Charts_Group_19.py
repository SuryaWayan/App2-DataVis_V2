...
# Show charts?
Chart_19 = st.session_state.get('Chart_19', False)
def handle_Chart_19():
    if st.session_state.handle_Chart_19:
        st.session_state.Chart_19 = st.session_state.handle_Chart_19
Chart_19 = st.checkbox("Click to Show charts", Chart_19, on_change=handle_Chart_19, key='handle_Chart_19')
st.session_state.Chart_19 = Chart_19
...
if Chart_19:
    ...
    start_x_all_19 = st.session_state.get('start_x_all_19', None)
    end_x_all_19 = st.session_state.get('end_x_all_19', None)
    tunnel_19 = st.session_state.get('tunnel_19', False)
    threshold_19 = st.session_state.get('threshold_19', 0.00000)
    C_19_option = st.session_state.get('C_19_option', None)
    C_19 = st.session_state.get('C_19', 0.00000)
    C_19_column = st.session_state.get('C_19_column', "Row Number")
    B_19 = st.session_state.get('B_19', 0.00000)
    D_19 = st.session_state.get('D_19', 0.00000)
    A_19 = st.session_state.get('A_19', [])
    ...
    if data is not None:
        ...
        if start_x_all_19 is None or end_x_all_19 is None:
            start_x_all_19 = x_min
            end_x_all_19 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_19, col_x_range_end_19, col_remain_19 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_19:
            def handle_start_x_all_19():
                if st.session_state.handle_start_x_all_19:
                    st.session_state.start_x_all_19 = st.session_state.handle_start_x_all_19
            start_x_all_19 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_19, on_change=handle_start_x_all_19, key='handle_start_x_all_19')
        with col_x_range_end_19:
            def handle_end_x_all_19():
                if st.session_state.handle_end_x_all_19:
                    st.session_state.end_x_all_19 = st.session_state.handle_end_x_all_19
            end_x_all_19 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_19, on_change=handle_end_x_all_19, key='handle_end_x_all_19')
        st.session_state.start_x_all_19 = start_x_all_19
        st.session_state.end_x_all_19 = end_x_all_19
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_19, B_19, C_19_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_19_option == "User Input":
                    C_19_value = C_19
                elif C_19_option == "From Existing Variables":
                    if "C_19_column" in data.columns:
                        C_19_value = data["C_19_column"]
                    else:
                        st.error("Please select a column for C_19_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_19_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_19 + (B_19 * C_19_value / 100)
                upper_limit[column_data <= threshold] += D_19 + (B_19 * C_19_value / 100)
                upper_limit[column_data > threshold] += B_19 * C_19_value / 100
                lower_limit[column_data > threshold] -= B_19 * C_19_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_19():
            if st.session_state.handle_tunnel_19:
                st.session_state.tunnel_19 = st.session_state.handle_tunnel_19
        tunnel_19 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_19, on_change=handle_tunnel_19, key='handle_tunnel_19')
        st.session_state.tunnel_19 = tunnel_19
            
        if tunnel_19:
            col_notef_19, col_noteg_19 = st.columns([0.05,3.5])
            with col_noteg_19:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_19, col_notea_19, col_noteb_19, col_notec_19, col_noted_19, col_notee_19 = st.columns([0.08,1,1,1,1,1])

            with col_notea_19:
                def handle_threshold_19():
                        if st.session_state.handle_threshold_19:
                            st.session_state.threshold_19 = st.session_state.handle_threshold_19
                threshold_19 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_19, on_change=handle_threshold_19, key='handle_threshold_19')
                st.session_state.threshold_19 = threshold_19

            with col_noteb_19:
                def handle_C_19_option():
                        if st.session_state.handle_C_19_option:
                            st.session_state.C_19_option = st.session_state.handle_C_19_option
                C_19_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_19_option == "User Input" else 1, on_change=handle_C_19_option, key='handle_C_19_option')
                st.session_state.C_19_option = C_19_option
                if C_19_option == "User Input":
                    C_19 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_19)
                    st.session_state.C_19 = C_19

                elif C_19_option == "From Existing Variables":
                    #C_19_column = st.session_state.get('C_19_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_19_column)
                    def handle_C_19_column():
                        if st.session_state.handle_C_19_column:
                            st.session_state.C_19_column = st.session_state.handle_C_19_column
                    C_19_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_19_column, key='handle_C_19_column')
                    st.session_state['C_19_column'] = C_19_column
                    data["C_19_column"] = data[C_19_column]

            with col_notec_19:
                def handle_B_19():
                        if st.session_state.handle_B_19:
                            st.session_state.B_19 = st.session_state.handle_B_19
                B_19 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_19, on_change=handle_B_19, key='handle_B_19')
                st.session_state.B_19 = B_19

            with col_noted_19:
                def handle_D_19():
                        if st.session_state.handle_D_19:
                            st.session_state.D_19 = st.session_state.handle_D_19
                D_19 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_19, on_change=handle_D_19, key='handle_D_19')
                st.session_state.D_19 = D_19
                
            with col_notee_19:
                def handle_A_19():
                    if st.session_state.handle_A_19:
                        st.session_state.A_19 = st.session_state.handle_A_19
                A_19 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_19, on_change=handle_A_19, key='handle_A_19')    
                st.session_state.A_19 = A_19
            ...
            for column in A_19:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_19, D_19, B_19, C_19_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_19 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_19):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_19]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_19) & (data['Row Number'] <= end_x_all_19)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_19, y0=slope*start_x_all_19+intercept,
                                                    x1=end_x_all_19, y1=slope*end_x_all_19+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_19, end_x_all_19, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_19.append(data_row)

            summary_data_19.extend(chart_summary_data_19)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_19)
        st.session_state.summary_data_19 = summary_data_19
