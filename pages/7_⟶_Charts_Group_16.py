...
# Show charts?
Chart_16 = st.session_state.get('Chart_16', False)
def handle_Chart_16():
    if st.session_state.handle_Chart_16:
        st.session_state.Chart_16 = st.session_state.handle_Chart_16
Chart_16 = st.checkbox("Click to Show charts", Chart_16, on_change=handle_Chart_16, key='handle_Chart_16')
st.session_state.Chart_16 = Chart_16
...
if Chart_16:
    ...
    start_x_all_16 = st.session_state.get('start_x_all_16', None)
    end_x_all_16 = st.session_state.get('end_x_all_16', None)
    tunnel_16 = st.session_state.get('tunnel_16', False)
    threshold_16 = st.session_state.get('threshold_16', 0.00000)
    C_16_option = st.session_state.get('C_16_option', None)
    C_16 = st.session_state.get('C_16', 0.00000)
    C_16_column = st.session_state.get('C_16_column', "Row Number")
    B_16 = st.session_state.get('B_16', 0.00000)
    D_16 = st.session_state.get('D_16', 0.00000)
    A_16 = st.session_state.get('A_16', [])
    ...
    if data is not None:
        ...
        if start_x_all_16 is None or end_x_all_16 is None:
            start_x_all_16 = x_min
            end_x_all_16 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_16, col_x_range_end_16, col_remain_16 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_16:
            def handle_start_x_all_16():
                if st.session_state.handle_start_x_all_16:
                    st.session_state.start_x_all_16 = st.session_state.handle_start_x_all_16
            start_x_all_16 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_16, on_change=handle_start_x_all_16, key='handle_start_x_all_16')
        with col_x_range_end_16:
            def handle_end_x_all_16():
                if st.session_state.handle_end_x_all_16:
                    st.session_state.end_x_all_16 = st.session_state.handle_end_x_all_16
            end_x_all_16 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_16, on_change=handle_end_x_all_16, key='handle_end_x_all_16')
        st.session_state.start_x_all_16 = start_x_all_16
        st.session_state.end_x_all_16 = end_x_all_16
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_16, B_16, C_16_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_16_option == "User Input":
                    C_16_value = C_16
                elif C_16_option == "From Existing Variables":
                    if "C_16_column" in data.columns:
                        C_16_value = data["C_16_column"]
                    else:
                        st.error("Please select a column for C_16_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_16_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_16 + (B_16 * C_16_value / 100)
                upper_limit[column_data <= threshold] += D_16 + (B_16 * C_16_value / 100)
                upper_limit[column_data > threshold] += B_16 * C_16_value / 100
                lower_limit[column_data > threshold] -= B_16 * C_16_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_16():
            if st.session_state.handle_tunnel_16:
                st.session_state.tunnel_16 = st.session_state.handle_tunnel_16
        tunnel_16 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_16, on_change=handle_tunnel_16, key='handle_tunnel_16')
        st.session_state.tunnel_16 = tunnel_16
            
        if tunnel_16:
            col_notef_16, col_noteg_16 = st.columns([0.05,3.5])
            with col_noteg_16:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_16, col_notea_16, col_noteb_16, col_notec_16, col_noted_16, col_notee_16 = st.columns([0.08,1,1,1,1,1])

            with col_notea_16:
                def handle_threshold_16():
                        if st.session_state.handle_threshold_16:
                            st.session_state.threshold_16 = st.session_state.handle_threshold_16
                threshold_16 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_16, on_change=handle_threshold_16, key='handle_threshold_16')
                st.session_state.threshold_16 = threshold_16

            with col_noteb_16:
                def handle_C_16_option():
                        if st.session_state.handle_C_16_option:
                            st.session_state.C_16_option = st.session_state.handle_C_16_option
                C_16_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_16_option == "User Input" else 1, on_change=handle_C_16_option, key='handle_C_16_option')
                st.session_state.C_16_option = C_16_option
                if C_16_option == "User Input":
                    C_16 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_16)
                    st.session_state.C_16 = C_16

                elif C_16_option == "From Existing Variables":
                    #C_16_column = st.session_state.get('C_16_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_16_column)
                    def handle_C_16_column():
                        if st.session_state.handle_C_16_column:
                            st.session_state.C_16_column = st.session_state.handle_C_16_column
                    C_16_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_16_column, key='handle_C_16_column')
                    st.session_state['C_16_column'] = C_16_column
                    data["C_16_column"] = data[C_16_column]

            with col_notec_16:
                def handle_B_16():
                        if st.session_state.handle_B_16:
                            st.session_state.B_16 = st.session_state.handle_B_16
                B_16 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_16, on_change=handle_B_16, key='handle_B_16')
                st.session_state.B_16 = B_16

            with col_noted_16:
                def handle_D_16():
                        if st.session_state.handle_D_16:
                            st.session_state.D_16 = st.session_state.handle_D_16
                D_16 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_16, on_change=handle_D_16, key='handle_D_16')
                st.session_state.D_16 = D_16
                
            with col_notee_16:
                def handle_A_16():
                    if st.session_state.handle_A_16:
                        st.session_state.A_16 = st.session_state.handle_A_16
                A_16 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_16, on_change=handle_A_16, key='handle_A_16')    
                st.session_state.A_16 = A_16
            ...
            for column in A_16:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_16, D_16, B_16, C_16_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_16 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_16):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_16]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_16) & (data['Row Number'] <= end_x_all_16)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_16, y0=slope*start_x_all_16+intercept,
                                                    x1=end_x_all_16, y1=slope*end_x_all_16+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_16, end_x_all_16, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_16.append(data_row)

            summary_data_16.extend(chart_summary_data_16)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_16)
        st.session_state.summary_data_16 = summary_data_16
