...
# Show charts?
Chart_14 = st.session_state.get('Chart_14', False)
def handle_Chart_14():
    if st.session_state.handle_Chart_14:
        st.session_state.Chart_14 = st.session_state.handle_Chart_14
Chart_14 = st.checkbox("Click to Show charts", Chart_14, on_change=handle_Chart_14, key='handle_Chart_14')
st.session_state.Chart_14 = Chart_14
...
if Chart_14:
    ...
    start_x_all_14 = st.session_state.get('start_x_all_14', None)
    end_x_all_14 = st.session_state.get('end_x_all_14', None)
    tunnel_14 = st.session_state.get('tunnel_14', False)
    threshold_14 = st.session_state.get('threshold_14', 0.00000)
    C_14_option = st.session_state.get('C_14_option', None)
    C_14 = st.session_state.get('C_14', 0.00000)
    C_14_column = st.session_state.get('C_14_column', "Row Number")
    B_14 = st.session_state.get('B_14', 0.00000)
    D_14 = st.session_state.get('D_14', 0.00000)
    A_14 = st.session_state.get('A_14', [])
    ...
    if data is not None:
        ...
        if start_x_all_14 is None or end_x_all_14 is None:
            start_x_all_14 = x_min
            end_x_all_14 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_14, col_x_range_end_14, col_remain_14 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_14:
            def handle_start_x_all_14():
                if st.session_state.handle_start_x_all_14:
                    st.session_state.start_x_all_14 = st.session_state.handle_start_x_all_14
            start_x_all_14 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_14, on_change=handle_start_x_all_14, key='handle_start_x_all_14')
        with col_x_range_end_14:
            def handle_end_x_all_14():
                if st.session_state.handle_end_x_all_14:
                    st.session_state.end_x_all_14 = st.session_state.handle_end_x_all_14
            end_x_all_14 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_14, on_change=handle_end_x_all_14, key='handle_end_x_all_14')
        st.session_state.start_x_all_14 = start_x_all_14
        st.session_state.end_x_all_14 = end_x_all_14
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_14, B_14, C_14_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_14_option == "User Input":
                    C_14_value = C_14
                elif C_14_option == "From Existing Variables":
                    if "C_14_column" in data.columns:
                        C_14_value = data["C_14_column"]
                    else:
                        st.error("Please select a column for C_14_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_14_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_14 + (B_14 * C_14_value / 100)
                upper_limit[column_data <= threshold] += D_14 + (B_14 * C_14_value / 100)
                upper_limit[column_data > threshold] += B_14 * C_14_value / 100
                lower_limit[column_data > threshold] -= B_14 * C_14_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_14():
            if st.session_state.handle_tunnel_14:
                st.session_state.tunnel_14 = st.session_state.handle_tunnel_14
        tunnel_14 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_14, on_change=handle_tunnel_14, key='handle_tunnel_14')
        st.session_state.tunnel_14 = tunnel_14
            
        if tunnel_14:
            col_notef_14, col_noteg_14 = st.columns([0.05,3.5])
            with col_noteg_14:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_14, col_notea_14, col_noteb_14, col_notec_14, col_noted_14, col_notee_14 = st.columns([0.08,1,1,1,1,1])

            with col_notea_14:
                def handle_threshold_14():
                        if st.session_state.handle_threshold_14:
                            st.session_state.threshold_14 = st.session_state.handle_threshold_14
                threshold_14 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_14, on_change=handle_threshold_14, key='handle_threshold_14')
                st.session_state.threshold_14 = threshold_14

            with col_noteb_14:
                def handle_C_14_option():
                        if st.session_state.handle_C_14_option:
                            st.session_state.C_14_option = st.session_state.handle_C_14_option
                C_14_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_14_option == "User Input" else 1, on_change=handle_C_14_option, key='handle_C_14_option')
                st.session_state.C_14_option = C_14_option
                if C_14_option == "User Input":
                    C_14 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_14)
                    st.session_state.C_14 = C_14

                elif C_14_option == "From Existing Variables":
                    #C_14_column = st.session_state.get('C_14_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_14_column)
                    def handle_C_14_column():
                        if st.session_state.handle_C_14_column:
                            st.session_state.C_14_column = st.session_state.handle_C_14_column
                    C_14_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_14_column, key='handle_C_14_column')
                    st.session_state['C_14_column'] = C_14_column
                    data["C_14_column"] = data[C_14_column]

            with col_notec_14:
                def handle_B_14():
                        if st.session_state.handle_B_14:
                            st.session_state.B_14 = st.session_state.handle_B_14
                B_14 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_14, on_change=handle_B_14, key='handle_B_14')
                st.session_state.B_14 = B_14

            with col_noted_14:
                def handle_D_14():
                        if st.session_state.handle_D_14:
                            st.session_state.D_14 = st.session_state.handle_D_14
                D_14 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_14, on_change=handle_D_14, key='handle_D_14')
                st.session_state.D_14 = D_14
                
            with col_notee_14:
                def handle_A_14():
                    if st.session_state.handle_A_14:
                        st.session_state.A_14 = st.session_state.handle_A_14
                A_14 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_14, on_change=handle_A_14, key='handle_A_14')    
                st.session_state.A_14 = A_14
            ...
            for column in A_14:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_14, D_14, B_14, C_14_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_14 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_14):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_14]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_14) & (data['Row Number'] <= end_x_all_14)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_14, y0=slope*start_x_all_14+intercept,
                                                    x1=end_x_all_14, y1=slope*end_x_all_14+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_14, end_x_all_14, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_14.append(data_row)

            summary_data_14.extend(chart_summary_data_14)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_14)
        st.session_state.summary_data_14 = summary_data_14
