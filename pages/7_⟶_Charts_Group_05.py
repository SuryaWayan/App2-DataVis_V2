...
# Show charts?
Chart_5 = st.session_state.get('Chart_5', False)
def handle_Chart_5():
    if st.session_state.handle_Chart_5:
        st.session_state.Chart_5 = st.session_state.handle_Chart_5
Chart_5 = st.checkbox("Click to Show charts", Chart_5, on_change=handle_Chart_5, key='handle_Chart_5')
st.session_state.Chart_5 = Chart_5
...
if Chart_5:
    ...
    start_x_all_5 = st.session_state.get('start_x_all_5', None)
    end_x_all_5 = st.session_state.get('end_x_all_5', None)
    tunnel_5 = st.session_state.get('tunnel_5', False)
    threshold_5 = st.session_state.get('threshold_5', 0.00000)
    C_5_option = st.session_state.get('C_5_option', None)
    C_5 = st.session_state.get('C_5', 0.00000)
    C_5_column = st.session_state.get('C_5_column', "Row Number")
    B_5 = st.session_state.get('B_5', 0.00000)
    D_5 = st.session_state.get('D_5', 0.00000)
    A_5 = st.session_state.get('A_5', [])
    ...
    if data is not None:
        ...
        if start_x_all_5 is None or end_x_all_5 is None:
            start_x_all_5 = x_min
            end_x_all_5 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_5, col_x_range_end_5, col_remain_5 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_5:
            def handle_start_x_all_5():
                if st.session_state.handle_start_x_all_5:
                    st.session_state.start_x_all_5 = st.session_state.handle_start_x_all_5
            start_x_all_5 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_5, on_change=handle_start_x_all_5, key='handle_start_x_all_5')
        with col_x_range_end_5:
            def handle_end_x_all_5():
                if st.session_state.handle_end_x_all_5:
                    st.session_state.end_x_all_5 = st.session_state.handle_end_x_all_5
            end_x_all_5 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_5, on_change=handle_end_x_all_5, key='handle_end_x_all_5')
        st.session_state.start_x_all_5 = start_x_all_5
        st.session_state.end_x_all_5 = end_x_all_5
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_5, B_5, C_5_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_5_option == "User Input":
                    C_5_value = C_5
                elif C_5_option == "From Existing Variables":
                    if "C_5_column" in data.columns:
                        C_5_value = data["C_5_column"]
                    else:
                        st.error("Please select a column for C_5_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_5_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_5 + (B_5 * C_5_value / 100)
                upper_limit[column_data <= threshold] += D_5 + (B_5 * C_5_value / 100)
                upper_limit[column_data > threshold] += B_5 * C_5_value / 100
                lower_limit[column_data > threshold] -= B_5 * C_5_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_5():
            if st.session_state.handle_tunnel_5:
                st.session_state.tunnel_5 = st.session_state.handle_tunnel_5
        tunnel_5 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_5, on_change=handle_tunnel_5, key='handle_tunnel_5')
        st.session_state.tunnel_5 = tunnel_5
            
        if tunnel_5:
            col_notef_5, col_noteg_5 = st.columns([0.05,3.5])
            with col_noteg_5:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_5, col_notea_5, col_noteb_5, col_notec_5, col_noted_5, col_notee_5 = st.columns([0.08,1,1,1,1,1])

            with col_notea_5:
                def handle_threshold_5():
                        if st.session_state.handle_threshold_5:
                            st.session_state.threshold_5 = st.session_state.handle_threshold_5
                threshold_5 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_5, on_change=handle_threshold_5, key='handle_threshold_5')
                st.session_state.threshold_5 = threshold_5

            with col_noteb_5:
                def handle_C_5_option():
                        if st.session_state.handle_C_5_option:
                            st.session_state.C_5_option = st.session_state.handle_C_5_option
                C_5_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_5_option == "User Input" else 1, on_change=handle_C_5_option, key='handle_C_5_option')
                st.session_state.C_5_option = C_5_option
                if C_5_option == "User Input":
                    C_5 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_5)
                    st.session_state.C_5 = C_5

                elif C_5_option == "From Existing Variables":
                    #C_5_column = st.session_state.get('C_5_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_5_column)
                    def handle_C_5_column():
                        if st.session_state.handle_C_5_column:
                            st.session_state.C_5_column = st.session_state.handle_C_5_column
                    C_5_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_5_column, key='handle_C_5_column')
                    st.session_state['C_5_column'] = C_5_column
                    data["C_5_column"] = data[C_5_column]

            with col_notec_5:
                def handle_B_5():
                        if st.session_state.handle_B_5:
                            st.session_state.B_5 = st.session_state.handle_B_5
                B_5 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_5, on_change=handle_B_5, key='handle_B_5')
                st.session_state.B_5 = B_5

            with col_noted_5:
                def handle_D_5():
                        if st.session_state.handle_D_5:
                            st.session_state.D_5 = st.session_state.handle_D_5
                D_5 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_5, on_change=handle_D_5, key='handle_D_5')
                st.session_state.D_5 = D_5
                
            with col_notee_5:
                def handle_A_5():
                    if st.session_state.handle_A_5:
                        st.session_state.A_5 = st.session_state.handle_A_5
                A_5 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_5, on_change=handle_A_5, key='handle_A_5')    
                st.session_state.A_5 = A_5
            ...
            for column in A_5:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_5, D_5, B_5, C_5_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_5 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_5):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_5]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_5) & (data['Row Number'] <= end_x_all_5)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_5, y0=slope*start_x_all_5+intercept,
                                                    x1=end_x_all_5, y1=slope*end_x_all_5+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_5, end_x_all_5, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_5.append(data_row)

            summary_data_5.extend(chart_summary_data_5)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_5)
        st.session_state.summary_data_5 = summary_data_5
