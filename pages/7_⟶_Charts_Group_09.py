...
# Show charts?
Chart_9 = st.session_state.get('Chart_9', False)
def handle_Chart_9():
    if st.session_state.handle_Chart_9:
        st.session_state.Chart_9 = st.session_state.handle_Chart_9
Chart_9 = st.checkbox("Click to Show charts", Chart_9, on_change=handle_Chart_9, key='handle_Chart_9')
st.session_state.Chart_9 = Chart_9
...
if Chart_9:
    ...
    start_x_all_9 = st.session_state.get('start_x_all_9', None)
    end_x_all_9 = st.session_state.get('end_x_all_9', None)
    tunnel_9 = st.session_state.get('tunnel_9', False)
    threshold_9 = st.session_state.get('threshold_9', 0.00000)
    C_9_option = st.session_state.get('C_9_option', None)
    C_9 = st.session_state.get('C_9', 0.00000)
    C_9_column = st.session_state.get('C_9_column', "Row Number")
    B_9 = st.session_state.get('B_9', 0.00000)
    D_9 = st.session_state.get('D_9', 0.00000)
    A_9 = st.session_state.get('A_9', [])
    ...
    if data is not None:
        ...
        if start_x_all_9 is None or end_x_all_9 is None:
            start_x_all_9 = x_min
            end_x_all_9 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_9, col_x_range_end_9, col_remain_9 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_9:
            def handle_start_x_all_9():
                if st.session_state.handle_start_x_all_9:
                    st.session_state.start_x_all_9 = st.session_state.handle_start_x_all_9
            start_x_all_9 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_9, on_change=handle_start_x_all_9, key='handle_start_x_all_9')
        with col_x_range_end_9:
            def handle_end_x_all_9():
                if st.session_state.handle_end_x_all_9:
                    st.session_state.end_x_all_9 = st.session_state.handle_end_x_all_9
            end_x_all_9 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_9, on_change=handle_end_x_all_9, key='handle_end_x_all_9')
        st.session_state.start_x_all_9 = start_x_all_9
        st.session_state.end_x_all_9 = end_x_all_9
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_9, B_9, C_9_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_9_option == "User Input":
                    C_9_value = C_9
                elif C_9_option == "From Existing Variables":
                    if "C_9_column" in data.columns:
                        C_9_value = data["C_9_column"]
                    else:
                        st.error("Please select a column for C_9_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_9_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_9 + (B_9 * C_9_value / 100)
                upper_limit[column_data <= threshold] += D_9 + (B_9 * C_9_value / 100)
                upper_limit[column_data > threshold] += B_9 * C_9_value / 100
                lower_limit[column_data > threshold] -= B_9 * C_9_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_9():
            if st.session_state.handle_tunnel_9:
                st.session_state.tunnel_9 = st.session_state.handle_tunnel_9
        tunnel_9 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_9, on_change=handle_tunnel_9, key='handle_tunnel_9')
        st.session_state.tunnel_9 = tunnel_9
            
        if tunnel_9:
            col_notef_9, col_noteg_9 = st.columns([0.05,3.5])
            with col_noteg_9:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_9, col_notea_9, col_noteb_9, col_notec_9, col_noted_9, col_notee_9 = st.columns([0.08,1,1,1,1,1])

            with col_notea_9:
                def handle_threshold_9():
                        if st.session_state.handle_threshold_9:
                            st.session_state.threshold_9 = st.session_state.handle_threshold_9
                threshold_9 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_9, on_change=handle_threshold_9, key='handle_threshold_9')
                st.session_state.threshold_9 = threshold_9

            with col_noteb_9:
                def handle_C_9_option():
                        if st.session_state.handle_C_9_option:
                            st.session_state.C_9_option = st.session_state.handle_C_9_option
                C_9_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_9_option == "User Input" else 1, on_change=handle_C_9_option, key='handle_C_9_option')
                st.session_state.C_9_option = C_9_option
                if C_9_option == "User Input":
                    C_9 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_9)
                    st.session_state.C_9 = C_9

                elif C_9_option == "From Existing Variables":
                    #C_9_column = st.session_state.get('C_9_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_9_column)
                    def handle_C_9_column():
                        if st.session_state.handle_C_9_column:
                            st.session_state.C_9_column = st.session_state.handle_C_9_column
                    C_9_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_9_column, key='handle_C_9_column')
                    st.session_state['C_9_column'] = C_9_column
                    data["C_9_column"] = data[C_9_column]

            with col_notec_9:
                def handle_B_9():
                        if st.session_state.handle_B_9:
                            st.session_state.B_9 = st.session_state.handle_B_9
                B_9 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_9, on_change=handle_B_9, key='handle_B_9')
                st.session_state.B_9 = B_9

            with col_noted_9:
                def handle_D_9():
                        if st.session_state.handle_D_9:
                            st.session_state.D_9 = st.session_state.handle_D_9
                D_9 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_9, on_change=handle_D_9, key='handle_D_9')
                st.session_state.D_9 = D_9
                
            with col_notee_9:
                def handle_A_9():
                    if st.session_state.handle_A_9:
                        st.session_state.A_9 = st.session_state.handle_A_9
                A_9 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_9, on_change=handle_A_9, key='handle_A_9')    
                st.session_state.A_9 = A_9
            ...
            for column in A_9:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_9, D_9, B_9, C_9_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_9 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_9):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_9]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_9) & (data['Row Number'] <= end_x_all_9)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_9, y0=slope*start_x_all_9+intercept,
                                                    x1=end_x_all_9, y1=slope*end_x_all_9+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_9, end_x_all_9, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_9.append(data_row)

            summary_data_9.extend(chart_summary_data_9)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_9)
        st.session_state.summary_data_9 = summary_data_9
