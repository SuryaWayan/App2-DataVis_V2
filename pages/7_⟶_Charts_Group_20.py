...
# Show charts?
Chart_20 = st.session_state.get('Chart_20', False)
def handle_Chart_20():
    if st.session_state.handle_Chart_20:
        st.session_state.Chart_20 = st.session_state.handle_Chart_20
Chart_20 = st.checkbox("Click to Show charts", Chart_20, on_change=handle_Chart_20, key='handle_Chart_20')
st.session_state.Chart_20 = Chart_20
...
if Chart_20:
    ...
    start_x_all_20 = st.session_state.get('start_x_all_20', None)
    end_x_all_20 = st.session_state.get('end_x_all_20', None)
    tunnel_20 = st.session_state.get('tunnel_20', False)
    threshold_20 = st.session_state.get('threshold_20', 0.00000)
    C_20_option = st.session_state.get('C_20_option', None)
    C_20 = st.session_state.get('C_20', 0.00000)
    C_20_column = st.session_state.get('C_20_column', "Row Number")
    B_20 = st.session_state.get('B_20', 0.00000)
    D_20 = st.session_state.get('D_20', 0.00000)
    A_20 = st.session_state.get('A_20', [])
    ...
    if data is not None:
        ...
        if start_x_all_20 is None or end_x_all_20 is None:
            start_x_all_20 = x_min
            end_x_all_20 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_20, col_x_range_end_20, col_remain_20 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_20:
            def handle_start_x_all_20():
                if st.session_state.handle_start_x_all_20:
                    st.session_state.start_x_all_20 = st.session_state.handle_start_x_all_20
            start_x_all_20 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_20, on_change=handle_start_x_all_20, key='handle_start_x_all_20')
        with col_x_range_end_20:
            def handle_end_x_all_20():
                if st.session_state.handle_end_x_all_20:
                    st.session_state.end_x_all_20 = st.session_state.handle_end_x_all_20
            end_x_all_20 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_20, on_change=handle_end_x_all_20, key='handle_end_x_all_20')
        st.session_state.start_x_all_20 = start_x_all_20
        st.session_state.end_x_all_20 = end_x_all_20
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_20, B_20, C_20_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_20_option == "User Input":
                    C_20_value = C_20
                elif C_20_option == "From Existing Variables":
                    if "C_20_column" in data.columns:
                        C_20_value = data["C_20_column"]
                    else:
                        st.error("Please select a column for C_20_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_20_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_20 + (B_20 * C_20_value / 100)
                upper_limit[column_data <= threshold] += D_20 + (B_20 * C_20_value / 100)
                upper_limit[column_data > threshold] += B_20 * C_20_value / 100
                lower_limit[column_data > threshold] -= B_20 * C_20_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_20():
            if st.session_state.handle_tunnel_20:
                st.session_state.tunnel_20 = st.session_state.handle_tunnel_20
        tunnel_20 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_20, on_change=handle_tunnel_20, key='handle_tunnel_20')
        st.session_state.tunnel_20 = tunnel_20
            
        if tunnel_20:
            col_notef_20, col_noteg_20 = st.columns([0.05,3.5])
            with col_noteg_20:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_20, col_notea_20, col_noteb_20, col_notec_20, col_noted_20, col_notee_20 = st.columns([0.08,1,1,1,1,1])

            with col_notea_20:
                def handle_threshold_20():
                        if st.session_state.handle_threshold_20:
                            st.session_state.threshold_20 = st.session_state.handle_threshold_20
                threshold_20 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_20, on_change=handle_threshold_20, key='handle_threshold_20')
                st.session_state.threshold_20 = threshold_20

            with col_noteb_20:
                def handle_C_20_option():
                        if st.session_state.handle_C_20_option:
                            st.session_state.C_20_option = st.session_state.handle_C_20_option
                C_20_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_20_option == "User Input" else 1, on_change=handle_C_20_option, key='handle_C_20_option')
                st.session_state.C_20_option = C_20_option
                if C_20_option == "User Input":
                    C_20 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_20)
                    st.session_state.C_20 = C_20

                elif C_20_option == "From Existing Variables":
                    #C_20_column = st.session_state.get('C_20_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_20_column)
                    def handle_C_20_column():
                        if st.session_state.handle_C_20_column:
                            st.session_state.C_20_column = st.session_state.handle_C_20_column
                    C_20_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_20_column, key='handle_C_20_column')
                    st.session_state['C_20_column'] = C_20_column
                    data["C_20_column"] = data[C_20_column]

            with col_notec_20:
                def handle_B_20():
                        if st.session_state.handle_B_20:
                            st.session_state.B_20 = st.session_state.handle_B_20
                B_20 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_20, on_change=handle_B_20, key='handle_B_20')
                st.session_state.B_20 = B_20

            with col_noted_20:
                def handle_D_20():
                        if st.session_state.handle_D_20:
                            st.session_state.D_20 = st.session_state.handle_D_20
                D_20 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_20, on_change=handle_D_20, key='handle_D_20')
                st.session_state.D_20 = D_20
                
            with col_notee_20:
                def handle_A_20():
                    if st.session_state.handle_A_20:
                        st.session_state.A_20 = st.session_state.handle_A_20
                A_20 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_20, on_change=handle_A_20, key='handle_A_20')    
                st.session_state.A_20 = A_20
            ...
            for column in A_20:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_20, D_20, B_20, C_20_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_20 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_20):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_20]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_20) & (data['Row Number'] <= end_x_all_20)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_20, y0=slope*start_x_all_20+intercept,
                                                    x1=end_x_all_20, y1=slope*end_x_all_20+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_20, end_x_all_20, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_20.append(data_row)

            summary_data_20.extend(chart_summary_data_20)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_20)
        st.session_state.summary_data_20 = summary_data_20
