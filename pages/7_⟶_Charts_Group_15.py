...
# Show charts?
Chart_15 = st.session_state.get('Chart_15', False)
def handle_Chart_15():
    if st.session_state.handle_Chart_15:
        st.session_state.Chart_15 = st.session_state.handle_Chart_15
Chart_15 = st.checkbox("Click to Show charts", Chart_15, on_change=handle_Chart_15, key='handle_Chart_15')
st.session_state.Chart_15 = Chart_15
...
if Chart_15:
    ...
    start_x_all_15 = st.session_state.get('start_x_all_15', None)
    end_x_all_15 = st.session_state.get('end_x_all_15', None)
    tunnel_15 = st.session_state.get('tunnel_15', False)
    threshold_15 = st.session_state.get('threshold_15', 0.00000)
    C_15_option = st.session_state.get('C_15_option', None)
    C_15 = st.session_state.get('C_15', 0.00000)
    C_15_column = st.session_state.get('C_15_column', "Row Number")
    B_15 = st.session_state.get('B_15', 0.00000)
    D_15 = st.session_state.get('D_15', 0.00000)
    A_15 = st.session_state.get('A_15', [])
    ...
    if data is not None:
        ...
        if start_x_all_15 is None or end_x_all_15 is None:
            start_x_all_15 = x_min
            end_x_all_15 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_15, col_x_range_end_15, col_remain_15 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_15:
            def handle_start_x_all_15():
                if st.session_state.handle_start_x_all_15:
                    st.session_state.start_x_all_15 = st.session_state.handle_start_x_all_15
            start_x_all_15 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_15, on_change=handle_start_x_all_15, key='handle_start_x_all_15')
        with col_x_range_end_15:
            def handle_end_x_all_15():
                if st.session_state.handle_end_x_all_15:
                    st.session_state.end_x_all_15 = st.session_state.handle_end_x_all_15
            end_x_all_15 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_15, on_change=handle_end_x_all_15, key='handle_end_x_all_15')
        st.session_state.start_x_all_15 = start_x_all_15
        st.session_state.end_x_all_15 = end_x_all_15
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_15, B_15, C_15_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_15_option == "User Input":
                    C_15_value = C_15
                elif C_15_option == "From Existing Variables":
                    if "C_15_column" in data.columns:
                        C_15_value = data["C_15_column"]
                    else:
                        st.error("Please select a column for C_15_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_15_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_15 + (B_15 * C_15_value / 100)
                upper_limit[column_data <= threshold] += D_15 + (B_15 * C_15_value / 100)
                upper_limit[column_data > threshold] += B_15 * C_15_value / 100
                lower_limit[column_data > threshold] -= B_15 * C_15_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_15():
            if st.session_state.handle_tunnel_15:
                st.session_state.tunnel_15 = st.session_state.handle_tunnel_15
        tunnel_15 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_15, on_change=handle_tunnel_15, key='handle_tunnel_15')
        st.session_state.tunnel_15 = tunnel_15
            
        if tunnel_15:
            col_notef_15, col_noteg_15 = st.columns([0.05,3.5])
            with col_noteg_15:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_15, col_notea_15, col_noteb_15, col_notec_15, col_noted_15, col_notee_15 = st.columns([0.08,1,1,1,1,1])

            with col_notea_15:
                def handle_threshold_15():
                        if st.session_state.handle_threshold_15:
                            st.session_state.threshold_15 = st.session_state.handle_threshold_15
                threshold_15 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_15, on_change=handle_threshold_15, key='handle_threshold_15')
                st.session_state.threshold_15 = threshold_15

            with col_noteb_15:
                def handle_C_15_option():
                        if st.session_state.handle_C_15_option:
                            st.session_state.C_15_option = st.session_state.handle_C_15_option
                C_15_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_15_option == "User Input" else 1, on_change=handle_C_15_option, key='handle_C_15_option')
                st.session_state.C_15_option = C_15_option
                if C_15_option == "User Input":
                    C_15 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_15)
                    st.session_state.C_15 = C_15

                elif C_15_option == "From Existing Variables":
                    #C_15_column = st.session_state.get('C_15_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_15_column)
                    def handle_C_15_column():
                        if st.session_state.handle_C_15_column:
                            st.session_state.C_15_column = st.session_state.handle_C_15_column
                    C_15_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_15_column, key='handle_C_15_column')
                    st.session_state['C_15_column'] = C_15_column
                    data["C_15_column"] = data[C_15_column]

            with col_notec_15:
                def handle_B_15():
                        if st.session_state.handle_B_15:
                            st.session_state.B_15 = st.session_state.handle_B_15
                B_15 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_15, on_change=handle_B_15, key='handle_B_15')
                st.session_state.B_15 = B_15

            with col_noted_15:
                def handle_D_15():
                        if st.session_state.handle_D_15:
                            st.session_state.D_15 = st.session_state.handle_D_15
                D_15 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_15, on_change=handle_D_15, key='handle_D_15')
                st.session_state.D_15 = D_15
                
            with col_notee_15:
                def handle_A_15():
                    if st.session_state.handle_A_15:
                        st.session_state.A_15 = st.session_state.handle_A_15
                A_15 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_15, on_change=handle_A_15, key='handle_A_15')    
                st.session_state.A_15 = A_15
            ...
            for column in A_15:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_15, D_15, B_15, C_15_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_15 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_15):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_15]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_15) & (data['Row Number'] <= end_x_all_15)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_15, y0=slope*start_x_all_15+intercept,
                                                    x1=end_x_all_15, y1=slope*end_x_all_15+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_15, end_x_all_15, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_15.append(data_row)

            summary_data_15.extend(chart_summary_data_15)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_15)
        st.session_state.summary_data_15 = summary_data_15
