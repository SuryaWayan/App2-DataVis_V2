...
# Show charts?
Chart_17 = st.session_state.get('Chart_17', False)
def handle_Chart_17():
    if st.session_state.handle_Chart_17:
        st.session_state.Chart_17 = st.session_state.handle_Chart_17
Chart_17 = st.checkbox("Click to Show charts", Chart_17, on_change=handle_Chart_17, key='handle_Chart_17')
st.session_state.Chart_17 = Chart_17
...
if Chart_17:
    ...
    start_x_all_17 = st.session_state.get('start_x_all_17', None)
    end_x_all_17 = st.session_state.get('end_x_all_17', None)
    tunnel_17 = st.session_state.get('tunnel_17', False)
    threshold_17 = st.session_state.get('threshold_17', 0.00000)
    C_17_option = st.session_state.get('C_17_option', None)
    C_17 = st.session_state.get('C_17', 0.00000)
    C_17_column = st.session_state.get('C_17_column', "Row Number")
    B_17 = st.session_state.get('B_17', 0.00000)
    D_17 = st.session_state.get('D_17', 0.00000)
    A_17 = st.session_state.get('A_17', [])
    ...
    if data is not None:
        ...
        if start_x_all_17 is None or end_x_all_17 is None:
            start_x_all_17 = x_min
            end_x_all_17 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_17, col_x_range_end_17, col_remain_17 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_17:
            def handle_start_x_all_17():
                if st.session_state.handle_start_x_all_17:
                    st.session_state.start_x_all_17 = st.session_state.handle_start_x_all_17
            start_x_all_17 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_17, on_change=handle_start_x_all_17, key='handle_start_x_all_17')
        with col_x_range_end_17:
            def handle_end_x_all_17():
                if st.session_state.handle_end_x_all_17:
                    st.session_state.end_x_all_17 = st.session_state.handle_end_x_all_17
            end_x_all_17 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_17, on_change=handle_end_x_all_17, key='handle_end_x_all_17')
        st.session_state.start_x_all_17 = start_x_all_17
        st.session_state.end_x_all_17 = end_x_all_17
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_17, B_17, C_17_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_17_option == "User Input":
                    C_17_value = C_17
                elif C_17_option == "From Existing Variables":
                    if "C_17_column" in data.columns:
                        C_17_value = data["C_17_column"]
                    else:
                        st.error("Please select a column for C_17_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_17_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_17 + (B_17 * C_17_value / 100)
                upper_limit[column_data <= threshold] += D_17 + (B_17 * C_17_value / 100)
                upper_limit[column_data > threshold] += B_17 * C_17_value / 100
                lower_limit[column_data > threshold] -= B_17 * C_17_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_17():
            if st.session_state.handle_tunnel_17:
                st.session_state.tunnel_17 = st.session_state.handle_tunnel_17
        tunnel_17 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_17, on_change=handle_tunnel_17, key='handle_tunnel_17')
        st.session_state.tunnel_17 = tunnel_17
            
        if tunnel_17:
            col_notef_17, col_noteg_17 = st.columns([0.05,3.5])
            with col_noteg_17:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_17, col_notea_17, col_noteb_17, col_notec_17, col_noted_17, col_notee_17 = st.columns([0.08,1,1,1,1,1])

            with col_notea_17:
                def handle_threshold_17():
                        if st.session_state.handle_threshold_17:
                            st.session_state.threshold_17 = st.session_state.handle_threshold_17
                threshold_17 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_17, on_change=handle_threshold_17, key='handle_threshold_17')
                st.session_state.threshold_17 = threshold_17

            with col_noteb_17:
                def handle_C_17_option():
                        if st.session_state.handle_C_17_option:
                            st.session_state.C_17_option = st.session_state.handle_C_17_option
                C_17_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_17_option == "User Input" else 1, on_change=handle_C_17_option, key='handle_C_17_option')
                st.session_state.C_17_option = C_17_option
                if C_17_option == "User Input":
                    C_17 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_17)
                    st.session_state.C_17 = C_17

                elif C_17_option == "From Existing Variables":
                    #C_17_column = st.session_state.get('C_17_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_17_column)
                    def handle_C_17_column():
                        if st.session_state.handle_C_17_column:
                            st.session_state.C_17_column = st.session_state.handle_C_17_column
                    C_17_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_17_column, key='handle_C_17_column')
                    st.session_state['C_17_column'] = C_17_column
                    data["C_17_column"] = data[C_17_column]

            with col_notec_17:
                def handle_B_17():
                        if st.session_state.handle_B_17:
                            st.session_state.B_17 = st.session_state.handle_B_17
                B_17 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_17, on_change=handle_B_17, key='handle_B_17')
                st.session_state.B_17 = B_17

            with col_noted_17:
                def handle_D_17():
                        if st.session_state.handle_D_17:
                            st.session_state.D_17 = st.session_state.handle_D_17
                D_17 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_17, on_change=handle_D_17, key='handle_D_17')
                st.session_state.D_17 = D_17
                
            with col_notee_17:
                def handle_A_17():
                    if st.session_state.handle_A_17:
                        st.session_state.A_17 = st.session_state.handle_A_17
                A_17 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_17, on_change=handle_A_17, key='handle_A_17')    
                st.session_state.A_17 = A_17
            ...
            for column in A_17:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_17, D_17, B_17, C_17_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_17 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_17):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_17]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_17) & (data['Row Number'] <= end_x_all_17)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_17, y0=slope*start_x_all_17+intercept,
                                                    x1=end_x_all_17, y1=slope*end_x_all_17+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_17, end_x_all_17, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_17.append(data_row)

            summary_data_17.extend(chart_summary_data_17)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_17)
        st.session_state.summary_data_17 = summary_data_17
