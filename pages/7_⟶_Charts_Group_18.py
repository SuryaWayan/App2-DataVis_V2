...
# Show charts?
Chart_18 = st.session_state.get('Chart_18', False)
def handle_Chart_18():
    if st.session_state.handle_Chart_18:
        st.session_state.Chart_18 = st.session_state.handle_Chart_18
Chart_18 = st.checkbox("Click to Show charts", Chart_18, on_change=handle_Chart_18, key='handle_Chart_18')
st.session_state.Chart_18 = Chart_18
...
if Chart_18:
    ...
    start_x_all_18 = st.session_state.get('start_x_all_18', None)
    end_x_all_18 = st.session_state.get('end_x_all_18', None)
    tunnel_18 = st.session_state.get('tunnel_18', False)
    threshold_18 = st.session_state.get('threshold_18', 0.00000)
    C_18_option = st.session_state.get('C_18_option', None)
    C_18 = st.session_state.get('C_18', 0.00000)
    C_18_column = st.session_state.get('C_18_column', "Row Number")
    B_18 = st.session_state.get('B_18', 0.00000)
    D_18 = st.session_state.get('D_18', 0.00000)
    A_18 = st.session_state.get('A_18', [])
    ...
    if data is not None:
        ...
        if start_x_all_18 is None or end_x_all_18 is None:
            start_x_all_18 = x_min
            end_x_all_18 = x_max

        st.write("Select X-axis range for all charts:")
        col_x_range_start_18, col_x_range_end_18, col_remain_18 = st.columns([0.6, 0.6, 5])
        with col_x_range_start_18:
            def handle_start_x_all_18():
                if st.session_state.handle_start_x_all_18:
                    st.session_state.start_x_all_18 = st.session_state.handle_start_x_all_18
            start_x_all_18 = st.number_input("Enter X-axis start", min_value=x_min, max_value=x_max, value=start_x_all_18, on_change=handle_start_x_all_18, key='handle_start_x_all_18')
        with col_x_range_end_18:
            def handle_end_x_all_18():
                if st.session_state.handle_end_x_all_18:
                    st.session_state.end_x_all_18 = st.session_state.handle_end_x_all_18
            end_x_all_18 = st.number_input("Enter X-axis end", min_value=x_min, max_value=x_max, value=end_x_all_18, on_change=handle_end_x_all_18, key='handle_end_x_all_18')
        st.session_state.start_x_all_18 = start_x_all_18
        st.session_state.end_x_all_18 = end_x_all_18
        st.write("")
        st.write("")

        # Function to calculate upper and lower limits for specified columns
        def calculate_limits(column_data, threshold, D_18, B_18, C_18_option, data):
            lower_limit = upper_limit = np.nan
            if pd.api.types.is_numeric_dtype(column_data):
                lower_limit = column_data.copy()
                upper_limit = column_data.copy()
                
                if C_18_option == "User Input":
                    C_18_value = C_18
                elif C_18_option == "From Existing Variables":
                    if "C_18_column" in data.columns:
                        C_18_value = data["C_18_column"]
                    else:
                        st.error("Please select a column for C_18_column.")
                        return np.nan, np.nan
                else:
                    st.error("Invalid option selected for C_18_option.")
                    return np.nan, np.nan
                
                lower_limit[column_data <= threshold] -= D_18 + (B_18 * C_18_value / 100)
                upper_limit[column_data <= threshold] += D_18 + (B_18 * C_18_value / 100)
                upper_limit[column_data > threshold] += B_18 * C_18_value / 100
                lower_limit[column_data > threshold] -= B_18 * C_18_value / 100
                
            return lower_limit, upper_limit

        # Adding Tunnel. Check if X and Y values are provided and the chart type is not Bar
        def handle_tunnel_18():
            if st.session_state.handle_tunnel_18:
                st.session_state.tunnel_18 = st.session_state.handle_tunnel_18
        tunnel_18 = st.checkbox("Add tunnel - lower & upper limit of variables", tunnel_18, on_change=handle_tunnel_18, key='handle_tunnel_18')
        st.session_state.tunnel_18 = tunnel_18
            
        if tunnel_18:
            col_notef_18, col_noteg_18 = st.columns([0.05,3.5])
            with col_noteg_18:
                st.write("     If A > threshold, tunnel = A ± (B% x C)")
                st.write("     If A ≤ threshold, tunnel = A ± (B% x C) ± D")

            col_noteh_18, col_notea_18, col_noteb_18, col_notec_18, col_noted_18, col_notee_18 = st.columns([0.08,1,1,1,1,1])

            with col_notea_18:
                def handle_threshold_18():
                        if st.session_state.handle_threshold_18:
                            st.session_state.threshold_18 = st.session_state.handle_threshold_18
                threshold_18 = st.number_input("Enter threshold value", min_value=0.00000, max_value=999999999.00000, value=threshold_18, on_change=handle_threshold_18, key='handle_threshold_18')
                st.session_state.threshold_18 = threshold_18

            with col_noteb_18:
                def handle_C_18_option():
                        if st.session_state.handle_C_18_option:
                            st.session_state.C_18_option = st.session_state.handle_C_18_option
                C_18_option = st.selectbox("Select option for C:", ["User Input", "From Existing Variables"], index=0 if C_18_option == "User Input" else 1, on_change=handle_C_18_option, key='handle_C_18_option')
                st.session_state.C_18_option = C_18_option
                if C_18_option == "User Input":
                    C_18 = st.number_input("Enter C", min_value=0.00000, max_value=999999999.00000, value=C_18)
                    st.session_state.C_18 = C_18

                elif C_18_option == "From Existing Variables":
                    #C_18_column = st.session_state.get('C_18_column', "Row Number")
                    numeric_columns = list(data.select_dtypes(include=np.number).columns)  
                    default_index = numeric_columns.index(C_18_column)
                    def handle_C_18_column():
                        if st.session_state.handle_C_18_column:
                            st.session_state.C_18_column = st.session_state.handle_C_18_column
                    C_18_column = st.selectbox("Select variables for C:", numeric_columns, index=default_index, on_change=handle_C_18_column, key='handle_C_18_column')
                    st.session_state['C_18_column'] = C_18_column
                    data["C_18_column"] = data[C_18_column]

            with col_notec_18:
                def handle_B_18():
                        if st.session_state.handle_B_18:
                            st.session_state.B_18 = st.session_state.handle_B_18
                B_18 = st.number_input("Enter B in %", min_value=0.00000, max_value=100.00000, value=B_18, on_change=handle_B_18, key='handle_B_18')
                st.session_state.B_18 = B_18

            with col_noted_18:
                def handle_D_18():
                        if st.session_state.handle_D_18:
                            st.session_state.D_18 = st.session_state.handle_D_18
                D_18 = st.number_input("Enter D", min_value=0.00000, max_value=999999999.00000, value=D_18, on_change=handle_D_18, key='handle_D_18')
                st.session_state.D_18 = D_18
                
            with col_notee_18:
                def handle_A_18():
                    if st.session_state.handle_A_18:
                        st.session_state.A_18 = st.session_state.handle_A_18
                A_18 = st.multiselect('Select A: variables to calculate limits for:', data.columns, A_18, on_change=handle_A_18, key='handle_A_18')    
                st.session_state.A_18 = A_18
            ...
            for column in A_18:
                ...
                if column in data.columns:
                    ...
                    lower_limit, upper_limit = calculate_limits(data[column], threshold_18, D_18, B_18, C_18_option, data)
        ...
        for i in range(num_charts_n):
            ...
            with col1_n:
                ...
                def handle_x_column_n(i):  
                    handle_key_x_18 = f'handle_x_columns_n_{i+1}'
                    if st.session_state.get(handle_key_x_18):
                        st.session_state[x_column_key_n] = st.session_state[handle_key_x_18]  
            ...
            with col_chart_n:
                with st.container():
                    if x_column_n and y_columns_n:
                        filtered_data = data[(data['Row Number'] >= start_x_all_18) & (data['Row Number'] <= end_x_all_18)]
                            ...
                            for trace in fig.data:
                                if trace.name in y_columns_n:
                                    ...
                                    if trendline_type_n == "Linear":
                                        ...
                                        fig.add_shape(type="line", x0=start_x_all_18, y0=slope*start_x_all_18+intercept,
                                                    x1=end_x_all_18, y1=slope*end_x_all_18+intercept,
                                        ...
                                        x_values = np.linspace(start_x_all_18, end_x_all_18, 100)
            ...
            for y_column in y_columns_n:
                ...
                chart_summary_data_18.append(data_row)

            summary_data_18.extend(chart_summary_data_18)  # Extend the summary data with the chart summary data

        # Convert the summary data into a DataFrame
        summary_df = pd.DataFrame(summary_data_18)
        st.session_state.summary_data_18 = summary_data_18
